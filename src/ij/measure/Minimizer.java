 ...
 * method), including the 'outside contraction' as described in: J.C. Lagarias,
 * J.A. Reeds, M.H. Wright, P. Wright: Convergence properties of the Nelder-Mead
 * simplex algorithm in low dimensions. SIAM J. Optim. 9, 112-147 (1998).
 * Differences w.r.t. this publication: - If outside contraction is rejected,
 * instead of shrinking the whole simplex, an inside contraction is tried first.
 * Own experiments show that this results in slightly better performance for
 * some test functions (Perm, Mc Kinnon's function with tau=2, theta=6, Osborne1
 * curve-fitting problem). In most cases, there is no difference, however. -
 * This implementation does not include any 'ordering rules' in case of equal
 * function values. - When checking for convergence, a special iteration step
 * may be performed, improving the best vertex of the simplex.
 *
 * Re-initialization within a minimization run: In some cases, the simplex
 * algorithm may fail to find the minimum or the convergence check might stop it
 * prematurely. Therefore, the search is initialized again, keeping the best
 * vertex of the simplex and setting the other vertices to random values, but
 * keeping variations of the parameters w.r.t. the best vertex at a similar
 * value. If re-initializing the simplex does not lead to a significant
 * improvement, the value is accepted as true (local) minimum.
 *
 * Multiple minimization runs: In spite of re-initializing (see above), there
 * are rare cases where minimization is stopped too early. Also, minimization
 * may result in a local minimum. Therefore, unless determined otherwise by
 * setting 'setRestarts', two minimization runs with different initialization of
 * the simplex are started in parallel threads. If the results don't agree
 * within the error limit, two more minimization runs are started. This is
 * repeated until the two best results agree within the error limits or the
 * number of restarts (determined by 'setRestarts'; default 2, i.e., up to 3
 * runs with two threads each) is exceeded. This does not guarantee that the
 * minimum is a global minimum, however: A local minimum will be accepted if the
 * minimizer finds a local minimum twice (or two different local minima with the
 * same function value within the error bounds), but no better minimum has been
 * found at that time.
 *
 * The user-supplied target function should return NaN for out-of-bounds
 * parameters instead of a high (penalty) value (minimization is faster and more
 * reliable with NaNs). The region where the function is defined (e.g. not
 * returning NaN) must be convex. Sharp corners of the region where the function
 * value is defined (especially in higher dimensions) may cause a problem with
 * finding suitable test points when (re-)initializing the simplex. If all
 * attempts to find initial points result in NaN, the status returned is
 * INITIALIZATION_FAILURE.
 *
 * Versions: Michael Schmid 2012-01-30: first version, based on previous
 * CurveFitter 2012-11-20: mor tries to find initial params not leading to NaN
 * 2013-09-24: 50% higher maximum iteration count, and never uses more than
 * 0.4*maxIter iterations per minimization to avoid trying too few sets of
 * initial params 2013-10-13: setStatusAndEscape to show iterations and enable
 * abort by ESC 2014-09-16: normalization bug fixed. makeNewParamVariations
 * checks for paramResolutions
 *
 */
public class Minimizer {
     ...
     * initialParams resulted in the target function returning NaN or all
     * attempts to find starting parameters for the other simplex points
     * resulted in the target function returning NaN. No minimization was
     * possible.
     */
     ...
     * to find restarting parameters resulted in the target function returning
     * NaN. Reinitialization is needed to obtain a reliable result; so the
     * result may be inaccurate or wrong.
     */
    ...
    private boolean useSingleThread
            = // single thread if one processor or set by useSingleThread(true)
     ...
     * the UserFunction interface). This function must allow simultaneous calls
     * in multiple threads unless setMaximumThreads(1); has been called. Note
     * that the function will be called with at least numParams+1 array
     ...
     * few times, depending on the value of 'restarts'. Running it several times
     * helps to reduce the probability of finding local minima or accepting one
     * of the rare results where the minimizer has got stuck before finding the
     * true minimum. We are using two threads and terminate after two equal
     * results. Thus, apart from the overhead of starting a new thread
     * (typically < 1 ms), for unproblematic minimization problems on a
     * dual-core machine this is almost as fast as running it once.
     *
     ...
     * (variables). When null, the starting values are assumed to be 0. The
     * target function must be defined (not returning NaN) for the values
     * specified as initialParams.
     * @param initialParamVariations Parameters (variables) are initially varied
     * by up to +/- this value. If not given (null), initial variations are
     * taken as 10% of initial parameter value or 0.01 for parameters that are
     * zero. When this array is given, all elements must be positive (nonzero).
     * If one or several initial parameters are zero, is advisable to set the
     * initialParamVariations array to useful values indicating the typical
     * order of magnitude of the parameters. For target functions with only one
     * minimum, convergence is fastest with large values of
     * initialParamVariations, so that the expected value is within
     * initialParam+/-initialParamVariations. If local minima can occur, it is
     * best to use a value close to the expected global minimum, and rather
     * small initialParamVariations, much lower than the distance to the nearest
     * local minimum.
     *
     * @return status code; SUCCESS if two attempts have found minima with the
     * same value (within the error bounds); so a minimum has been found with
    ...
    public int minimize(final double[] initialParams, final double[] initialParamVariations) {
        ...
        return maxRestarts > 0
                ...
                : // number of restarts exceeded without two equal results
     ...
     * re-initialization until we have a stable solution. Use the 'getParams'
     ...
     * specified as initialParams.
     * @param initialParamVariations Parameters (variables) are initially varied
     * by up to +/- this value. If not given (null), iniital variations are
     * taken as 10% of inial parameter value or 0.01 for parameters that are
     * zero. When this array is given, all elements must be positive (nonzero).
     * If one or several initial parameters are zero, is advisable to set the
     * initialParamVariations array to useful values indicating the typical
     * order of magnitude of the parameters. For target functions with only one
     ...
     * have more elements than numParams; ignore the rest. May return an array
     * with only NaN values in case the minimize call has returned an
     * INITIALIZATION_FAILURE status or that abort() has been called at the very
     * beginning of the minimization. Do not call this method before
     * minimization.
     */
     ...
     * resulting parameters as obtained by getParams(). May return NaN in case
     * the minimize call has returned an INITIALIZATION_FAILURE status or that
     * abort() has been called at the very beginning of the minimization. Do not
     * call this method before minimization.
     */
     ...
     * needs between one and numParams+3 calls of the target function (typically
     ...
     * threads). The number of function calls will be higher, up to about twice
     * the number of iterations. Note that the number of iterations needed
     * typically scales with the square of the dimensions (i.e., numParams^2).
     * Default value is 1000 * numParams^2 (half that value if maxRestarts=0),
     * which is enough for >99% of all cases (if the maximum number of restarts
     * is set to 2); typical number of iterations are below 10 and 20% of this
     * value.
     */
     ...
     * 'setMaxIterations', this value is defined only after running
     * 'setFunction'
     */
    ...
    /**
     * Set maximum number of minimization restarts to do. With n=0, the
     * minimizer is run once in a single thread. With n>0, two threads are used,
     * and if the two results do not agree within the error bounds, additional
     * optimizations are done up to n times, each with two threads. In any case,
     * if the two best results are within the error bounds, the best result is
     * accepted. Thus, on dual-core machines running no other jobs, values of
     * n=1 or n=2 (default) do not cause a notable increase of computing time
     * for 'easy' optimization problems, while greatly reducing the risk of
     * running into spurious local minima or non- optimal results due to the
     * minimizer getting stuck. In problematic cases, the improved The 'n' value
     * does not refer to the restarts within one minimization run (there, at
     * least one restart is performed, and restart is repeated until the result
     * does not change within the error bounds). This value does not affect the
     * 'minimizeOnce' function call. When setting the maximum number of restarts
     * to a value much higher than 3, remember to adjust the maximum number of
     * iterations (see setMaxIterations).
     */
     ...
     * because the number of minimization was exceeded). After a minimize(..)
     * call, typically 2 for unproblematic cases. Higher number indicate a
     * functin that is difficult to minimize or the existence of more than one
     * minimum.
     */
     ...
     * 1e-10) and the maximum absolute error is more than maxAbsError (i.e. it
     * is enough to fulfil one of these two criteria)
     */
     ...
     * function is not smooth but suffers from numerical noise. If all
     * parameters of all vertices are closer to the best value than the
     * respective resolution value, minimization is finished, irrespective of
     * the difference of the target function values at the vertices
     */
     ...
     * user-provided target function does not allow moultithreading). Currently
     * a maximum of 2 thread is used irrespective of any higher value.
     */
     ...
     * found so far. This method may be called from the user-supplied target
     * function. If displayStatusAndCheckEsc has been called before, the
     * Minimizer itself checks for the ESC key.
     */
    public void abort() {
        status = ABORTED;
    }
     ...
     *
     * @param ijStatusString Displayed in the beginning of the status message.
     * No display if null. E.g. "Optimization: Iteration "
     * @param checkEscape When true, the Minimizer stops if escape is pressed
     * and the status becomes ABORTED. Note that checking for ESC does not work
     * in the Event Queue thread.
     */
     ...
     * vaiables) for private use in the user function. Note that the first
     * numParams+1 elements should not be touched.
     */
     ...
     * target function wants to modify the simplex
     */
    ...
    /**
     * Minimizes the target function by variation of the simplex. Note that one
     * call to this function never does more than 0.4*maxIter iterations.
     *
    ...
    private int minimize(double[][] simp) {
        ...
        while (true) {
            ...
            boolean checkParamResolution
                    = // if new 'worst' is not close to 'best', don't check any further
     ...
     * worstVertex The new point is written to'newVertex' and target function is
     * evaluated there
     */
    ...
    /**
     * Get center of all vertices except one to exclude (may be -1 to exclude
     * none) Does not care about function values (i.e., last array elements)
     */
    ...
    /**
     * Whether the distance between the best and any other simplex vertex is
     * less than the resolution of the parameters
     */
     ...
     * yield NaN Called with params containing the initial parameters that have
    ...
    private void findValidInitalParams(double[] params, double[] initialParamVariations, Random random) {
        ...
        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            for (int i = 0; i < numParams; i++) {
                double multiplier = attempt < maxAttempts / 10 ? 1
                        : //after a while try different orders of magnitude
     ...
     * one, keeping the rough size of the simplex. This helps to avoid premature
     * termination or cases where the simplex has become degenerate. All points
     * of the new simplex are evaluated already. Returns false on error (if it
     * could not create a simplex not resulting in NaN).
     */
     ...
     * starting point. All points of the new simplex are evaluated already.
    ...
    /**
     * Calculate ranges for parameter variations for re-initializing: Note that
     * the simplex should have adapted in size to the useful range of each
     ...
     * may happen that one parameter has the same value for all vertices of the
     * simplex].
     *
     * For each parameter, as a variation value we use the typical difference
     * w.r.t. the best vertex of the simplex. To avoid a degenerate simplex, we
     * have to know what the the relative orders of magnitude of the parameter
     * variations should be. If initialParamVariations are given, we use them as
     * indication of the typoical relative values; otherwise we use the
     * parameter values themselves and assume that the variation of each
     * parameter should be roughly the same fraction of the value. If the
     * variation of a parameter in the simplex w.r.t. this estimated variation
     * is much smaller (<10^3) than typical for the parameters, we increase the
    ...
    /**
     * Evaluate the target function for the parameters given by 'vertex' The
     * function value is stored into the last (extra) array element.
     */
    ...
    /**
     * Function value of a vertex after call to 'evaluate' (stored it as last
     * array element)
     */
    ...
    /**
     * Find the simplex vertices with the worst, nextWorst and best values of
     * the target function
     */
