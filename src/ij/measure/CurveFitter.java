 ...
 * erf (=integral over Gaussian) 2021-04-30: data points can have weights
 */
public class CurveFitter implements UserFunction {
    ...
    /**
     * @deprecated now in the Minimizer class (since ImageJ 1.46f). (probably of
     * not much value for anyone anyhow?)
     */
    ...
    /**
     * Perform curve fitting with one of the built-in functions doFit(fitType)
     * does the fit quietly Use getStatus() and/or getStatusString() to see
     * whether fitting was (probably) successful and getParams() to access the
     * result.
     */
    ...
    /**
     * Perform curve fitting with one of the built-in functions doFit(fitType,
     * true) pops up a dialog allowing the user to set the initial fit
     * parameters and various numbers controlling the Minimizer Use getStatus()
     * and/or getStatusString() to see whether fitting was (probably) successful
     * and getParams() to access the result.
     */
    ...
    /**
     * Fit a function defined as a macro String like "y = a + b*x + c*x*x". When
     * showSettings is true, pops up a dialog allowing the user to set the
     * initial fit parameters and various numbers controlling the Minimizer
     ...
     * doCustomFit method with a (java) UserFunction, which also has more
     * options.
     *
     */
     ...
     * setOffsetMultiplySlopeParams, to avoid searching for one or two
     * parameters that can be calculated directly by linear regression.
     *
     * @param userFunction A class instance implementing the userFunction
     * interface. There, the fit function hould be defined by the method
     * userFunction(params, x). This function must allow simultaneous calls in
     * multiple threads.
     * @param numParams	Number of parameters of the fit function.
     * @param formula	A String describing the fit formula, may be null.
     * @param initialParams	Starting point for the parameters; the fit function
     * with these parameters must not return NaN for any of the data points
     * given in the constructor (xData). initialParams may be null, then random
     * values are used, with repeated tries if the userFunction returns NaN.
     * @param initialParamVariations Each parameter is initially varied by up to
     * +/- this value. If not given (null), initial variations are taken as 10%
     * of initial parameter value or 0.01 for parameters that are zero. When
     * this array is given, all elements must be positive (nonzero). See
     * Minimizer.minimize for details. Providing this array is especially
     * valuable if one or more initial parameters have a value of 0.
     * @param showSettings Displays a popup dialog for modifying the initial
     * parameters and a few numbers controlling the minimizer.
     */
     ...
     * length as the data arrays passed with the constructor. If the error bars
     * of the data points are known, the weights should be proportional to
     * 1/error^2. When weights are specified, note that 'getSumResidualsSqr'
     * will return the weighted sum.
     */
     ...
     * methods directly. Note that no Minimizer is used if fitType is any of
     * STRAIGHT_LINE, EXP_REGRESSION, and POWER_REGRESSION.
     */
     ...
     *
     * @param offsetParam Index of a parameter that is a pure offset: E.g. '0'
     * if	f(p0, p1, p2...) = p0 + function(p1, p2, ...).
     ...
     * summed to the function. E.g. '1' for f(p0, p1, p2, p3...) = p1*x +
     * func(p0, p2, p3, ...) Only one, multiplyParam and slopeParam can be used
     * (ie.e, the other should be set to -1)
     */
    ...
    /**
     * Get number of parameters for current fit formula Do not use before
     ...
     * 'custom' fit, where the number of parameters is given by the equation:
     * see getNumParams(String)
     */
     ...
     * String, like "y = a + b*x + c*x*x" . Restricted to 6 parameters "a" ...
     * "f" (fitting more parameters is not likely to yield an accurate result
     * anyhow). Returns 0 if a very basic check does not find a formula of this
     * type.
     */
     ...
     * fit. Note that the array returned may have more elements than numParams;
     * ignore the rest. May return an array with only NaN values if the
     * minimizer could not start properly, i.e., if getStatus() returns
     * Minimizer.INITILIZATION_FAILURE. See Minimizer.getParams() for details.
     */
     ...
     * 'Rodbard NIH Image' ). This is in contrast to sum of squared residuals,
     * which is for the fit that was actually done.
     */
    ...
    /**
     * Returns the sum of the residuals (may be NaN if the minimizer could not
     * start properly i.e., if getStatus() returns
     * Minimizer.INITILIZATION_FAILURE). If weights have been specified, each of
     * the residuals is multiplied by the corresponding weight before summing.
     */
    ...
    /**
     * Returns the standard deviation of the residuals. Here, the standard
     * deviation is defined here as the root-mean-square of the residuals times
     * sqrt(n/(n-1)); where n is the number of points. If weights are provided,
     * the standard deviation does not take the weights into account. With
     * weights, the standard deviation and getSumResidualsSqr (which uses
     * weights) are not related the usual way.
     */
    public double getSD() {
     ...
     * </pre> For power, exp by linear regression and 'Rodbard NIH Image', this
     * is calculated for the fit actually done, not for the residuals of the
     * original data.
     */
    ...
    /**
     * Get a measure of "goodness of fit" where 1.0 is best. Approaches R^2 if
     * the number of points is much larger than the number of fit parameters.
     * Assumes that the data points are independent (i.e., each point having a
     * different x value). For power, exp by linear regression and 'Rodbard NIH
     * Image', this is calculated for the fit actually done, not for the
     * residuals of the original data.
     */
     ...
     * preferred over Minimizer.STATUS_STRING[getMinimizer().getStatus()]
     * because getStatusString() better explains the problem in some cases of
     * initialization failure (data not compatible with the fit function chosen)
     */
     ...
     * (several lines, including the fit parameters).
     */
     ...
     * No display if null. E.g. "Curve Fit: Iteration "
     ...
     * indicates a successful completion. In case of
     * Minimizer.INITIALIZATION_FAILURE, fitting could not be performed because
     * the data and/or initial parameters are not compatible with the function
     * value. In that case, getStatusString may explain the problem. For further
     * status codes indicating problems during fitting, see the status codes of
     * the Minimzer class.
     */
    /**
    ...
    /**
     * Returns macro code of the form "y = ...x" for the fit function used. Note
     * that this is not neccessarily the equation acutally used for the fit (for
     * the various "linear regression" types and RODBARD2, the fit is done
     ...
     * squared residuals for given parameters. To improve the efficiency, simple
     * linear dependencies are solved directly by linear regression; in that
     * case the corresponding parameters are modified. This effectively reduces
     * the number of free parameters by one or two and thereby significantly
     * improves the performance of minimization.
     */
     ...
     * parameters handled by the Minimizer, convert Minimizer parameters to the
     * complete set of parameters. When not for calculating regression, we use
     * the sum of squared residuals, offset and multiplication factor stored in
     * the extra array elements: The Minimizer stores the sum of squared
     * residuals directly behind its last parameter. The next element is the
     * value of the offsetParam (if any). The final element is the value of the
     * factorParam (if any).
     */
    ...
    /**
     * Determine sum of squared residuals with linear regression. The sum of
     * squared residuals is written to the array element with index 'numParams',
     ...
     * sum of residuals squared. The last array elements, not used by the
     * minimizer, are the offset and factor parameters (if any)
     */
     ...
     * 'fitType' is still the type of the original (unmodified) fit. Also checks
     * for x values being non-negative for fit types that require this, and
     * returns false if the data cannot be fitted because of negative x. In such
     * a case, 'errorString' contains a message about the problem.
     */
     ...
     * to use linear regression, reducing the number of parameters used by the
     ...
     * Returns false if the data are incompatible with the fit type In that
     * case, 'errorString' is set to a message explaining the problem
     */
