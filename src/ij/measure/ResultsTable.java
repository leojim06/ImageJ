...
public class ResultsTable implements Cloneable {
     ...
     * precision set to 3 or the "Decimal places" value in Analyze/Set
     * Measurements if that value is higher than 3.
     */
    public ResultsTable() {
        init();
    }
    ...
    /**
     * Adds a numeric value to the specified column, on the last table row. If
     * the column does not exist, it is created. Use addRow() to add another row
     * to the table.
     * <p>
     * JavaScript example:
     * <pre>
    ...
    /**
     * Adds a string value to the specified column, on the last table row. If
     * the column does not exist, it is created. Use addRow() to add another row
     * to the table.
     *
     * @see #addRow
    ...
    /**
     * Adds a label to the beginning of the specified row, or updates an
     * existing lable, where 0<=row<size(). After labels are added or modified,
     * call <code>show()</code>
    ...
    /**
     * Returns a copy of the given column as a String array, or null if the
     * column is not found.
     */
    ...
    /**
     * Returns the value of the given column and row, where column must be less
     * than or equal the value returned by getLastColumn() and row must be
     ...
     * than value returned by size(). Throws an IllegalArgumentException if this
    ...
    /**
     * Returns the string value of the given column and row, where column must
    ...
    /**
     * Returns the label of the specified row. Returns null if the row does not
     * have a label.
     */
     ...
     * 0&lt;=row&lt;size(). If the specified column does not exist, it is
     * created. When adding columns, <code>show()</code> must be called to
    ...
    /**
     * Sets the values of the given column to the values in the array. If the
     * specified column does not exist, it is created. When adding columns,
     * <code>show()</code> must be called to update the window that displays the
     * table. If the array is shorter than the column length, the remaining
     * values of the column are left unchanged. If the array is longer, the
     * table is extended. String values are unaffected, but only used if the
     * numeric value at the given position is NaN.
     */
    ...
    /**
     * Sets the decimal places (digits to the right of decimal point) that are
     * used when this table is displayed.
     */
    ...
    /**
     * This is a version of IJ.d2s() that uses scientific notation for small
     * numbes that would otherwise display as zero.
     */
    ...
    /**
     * Adds the last row in this table to the Results window without updating
     * it.
     */
     ...
     * is no open text window with this title. The title must be "Results" if
     * this table was obtained using ResultsTable.getResultsTable() or
     ...
     * nothing if the table is empty. Displays an error message and returns
    ...
    /**
     * Applies a macro to each row of the table; the columns are assigned
     * variable names as given by getHeadingsAsVaribleNames(). New variables
     * starting with an uppercase letter create a new column with this name. The
     * variable 'row' (the row index) is pre-defined. Except for the row label
     * (if existing), currently only supports numeric values, no Strings.
     *
    ...
    /**
     * Returns the first index of a given non-null String in a String array, or
     * -1 if not found
     */
     ...
     * converted to valid variable names by replacing non-fitting characters
     * with underscores and adding underscores. To make unique names,
     * underscores+numbers are added as required.
     */
    ...
    /**
     * Converts a list of column headings to a list of corresponding variable
     * names
     */
    ...
    /**
     * Selects the row in the "Results" table assocuiated with the specified
     * Roi. The row number is obtained from the roi name..
     */
