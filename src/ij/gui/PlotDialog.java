...
public class PlotDialog implements DialogListener {
    ...
    /**
     * Template "copy what" flag: dialog texts and corresponding bit masks, in
     * the sequence as they appear in the dialog
     */
    ...
    /**
     * Setting up the dialog fields and initial parameters. The input is read in
     * the dialogItemChanged method, which must have exactly the same structure
     * of 'if' blocks and matching 'get' methods for each input field.
     *
     ...
     * that the 'if's for reading the fields must be exactly the same as those
     * for setting up the fields in 'setupDialog' (fields must be also read in
     * the same sequence).
     */
    ...
    private void doHighResolutionDialog(Frame parent) {
        ...
        /**
         * The following command is needed to have the high-resolution plot as
         * front window. Otherwise, as the dialog is owned by the original
         * PlotWindow, the WindowManager will see the original plot as active,
         * but the user interface will show the high-res plot as foreground
         * window
         */
