...
public class Roi extends Object implements Cloneable, java.io.Serializable, Iterable<Point> {
    ...
    /**
     * Starts the process of creating a user-defined rectangular Roi, where sx
     * and sy are the starting screen coordinates.
     */
    ...
    /**
     * Starts the process of creating a user-defined rectangular Roi, where sx
     * and sy are the starting screen coordinates. For rectangular rois, also a
     * corner diameter may be specified to make it a rounded rectangle
     */
    ...
    /**
     * Sets the ImagePlus associated with this ROI. <code>imp</code> may be null
     * to remove the association to an image.
     */
     ...
     * <br>[1] "FeretAngle" (angle of diameter with maximum caliper width,
     * between 0 and 180 deg)
     * <br>[2] "MinFeret" (minimum caliper width)
     * <br>[3][4] , "FeretX" and "FeretY", the X and Y coordinates of the
     * starting point (leftmost point) of the maximum-caliper-width diameter.
     * <br>[5-7] reserved
     * <br>All these values and point coordinates are in calibrated image
     * coordinates.
     * <p>
     * The following array elements are end points of the maximum and minimum
     * caliper diameter, in unscaled image pixel coordinates:
     * <br>[8][9] "FeretX1", "FeretY1"; unscaled versions of "FeretX" and
     * "FeretY" (subclasses may use any end of the diameter, not necessarily the
     * left one)
     * <br>[10][11] "FeretX2", "FeretY2", end point of the maxium-caliper-width
     * diameter. Both of these points are vertices of the convex hull.
     * <br> The final four array elements are the starting and end points of the
     * minimum caliper width,
     * <br>[12],[13] "MinFeretX", "MinFeretY", and
     * <br>[14],[15] "MinFeretX2", "MinFeretY2". These two pooints are not
     * sorted by x, but the first point point (MinFeretX, MinFeretY) is
     * guaranteed to be a vertex of the convex hull, while second point
     * (MinFeretX2, MinFeretY2) usually is not a vertex point but at a boundary
     ...
     * by rounding the floating-point values. Coordinates of the convex hull are
    ...
    /**
     * Sets the bounds of rectangular, oval or text selections. Note that for
     * these types, subpixel resolution is ignored, and the x,y values are
     * rounded down, the width and height values rounded up. Do not use for
     * other ROI types since their width and height are results of a
     * calculation. For translating ROIs, use setLocation.
     */
     ...
     * boundaries, such that all points are within the integer bounding
     * rectangle. For open line selections and (multi)Point Rois, note that
     * integer Roi coordinates correspond to the center of the 1x1 rectangle
     * enclosing a pixel. Points at the boundary of such a rectangle are counted
     * for the higher x or y value, in agreement to how (poly-)line or PointRois
     * are displayed at the screen at high zoom levels. (For lines and points,
     * it should include all pixels affected by 'draw'
     */
     ...
     * "close" to add a point to close the outline if this is an area roi and
     * the outline is not closed yet. (For ShapeRois, "separate" inserts NaN
     * values between subpaths).
     */
     ...
     * that has points spaced 1.0 pixel apart.
     */
    ...
    /**
     * Returns, as a FloatPolygon, an interpolated version of this selection
     * with points spaced 'interval' pixels apart. If 'smooth' is true, traced
    ...
    /**
     * Returns, as a FloatPolygon, an interpolated version of this selection
     * with points spaced abs('interval') pixels apart. If 'smooth' is true,
     * traced and freehand selections are first smoothed using a 3 point running
     * average. If 'interval' is negative, the program is allowed to decrease
     * abs('interval') so that the last segment will hit the end point
     */
    ...
    private void activeHandleSwitch(int x2, int y2, int xc, int yc) {
        switch (activeHandle) {
            case 0:
                x = x2 - width;
                y = y2 - height;
    ...
    protected void moveHandle(int sx, int sy) {
        ...
        if (constrain) {
            ...
            activeHandleSwitch(x2, y2, xc, yc);
    ...
    /**
     * Nudge lower right corner of rectangular and oval ROIs by one pixel based
     * on arrow key press.
     */
     ...
     * exactly at the left (right) border are considered outside (inside);
     * points exactly on horizontal borders are considered outside (inside) at
     * the border with the lower (higher) y. This convention is opposite to that
     * of the java.awt.Shape class.
     */
    ...
    /**
     * If 'add' is true, adds this selection to the previous one. If 'subtract'
     * is true, subtracts it from the previous selection. Called by the
     * IJ.doWand() method, and the makeRectangle(), makeOval(), makePolygon()
     * and makeSelection() macro functions.
     */
     ...
     * default ROI color to the group color. Set to zero to not have a default
     * group and to use the default ROI color.
     *
     * @see #setGroup
     * @see #getGroup
     * @see #getGroupColor
    ...
    /**
     * Sets the fill color used to display this ROI, or set to null to display
     * it transparently.
     *
     * @see #getFillColor
     * @see #setStrokeColor
    ...
    /**
     * Set 'nonScalable' true to have TextRois in a display list drawn at a
    ...
    /**
     * Sets the width of the line used to draw this ROI. Set the width to 0.0
     * and the ROI will be drawn using a a 1 pixel stroke width regardless of
     * the magnification.
     *
    ...
    /**
     * Sets the width of the line used to draw this ROI and prevents the width
     * from increasing when the image is zoomed.
     */
    ...
    /**
     * Sets the position of this ROI based on the stack position of the
     * specified image.
     */
     ...
     * shifted by 0.5 pixels top the bottom and right, for better agreement with
     * the position used by ProfilePlot, with the default taken from
     * Prefs.subPixelResolution. Now the shift is independent of this setting
     * and only depends on the ROI type (area or line/point ROI).
     */
    @Deprecated
    ...
    /**
     * Converts image canvas screen x coordinates to integer offscreen image
    ...
    /**
     * Converts image canvas screen y coordinates to integer offscreen image
     * pixel coordinates, depending on whether this roi uses the line or area
     * convention for coordinates.
     */
    ...
    /**
     * Converts image canvas screen x coordinates to floating-point offscreen
     * image pixel coordinates, depending on whether this roi uses the line or
     * area convention for coordinates.
     */
    ...
    /**
     * Converts image canvas screen y coordinates to floating-point offscreen
     * image pixel coordinates, depending on whether this roi uses the line or
     * area convention for coordinates.
     */
    ...
    /**
     * Returns 'true' if this ROI uses for drawing the convention for line and
     * point ROIs, where the coordinates are with respect to the pixel center.
     * Returns false for area rois, which have coordinates with respect to the
     * upper left corners of the pixels
     */
    ...
    /**
     * Returns whether a roi created interactively should have subpixel
     * resolution, (if the roi type supports it), i.e., whether the
    ...
    /**
     * Converts a line selection into an area (polygon or composite)
     * selection.<br>
    ...
    /**
     * Required by the {@link Iterable} interface. Use to iterate over the
     * contained coordinates. Usage example:
     * <pre>
     * for (Point p : roi) {
     *   // process p
     * }
     * </pre> Author: Wilhelm Burger
     *
     * @see #getContainedPoints()
     * @see #getContainedFloatPoints()
     */
