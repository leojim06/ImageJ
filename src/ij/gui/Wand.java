 ...
 * of equal or similar value or thresholded pixels forming a contiguous area.
 * The wand creates selections that have only one boundary line (inner holes are
 * not excluded from the selection). There may be holes at the boundary,
...
public class Wand {
    ...
    /**
     * Wand operation type similar to that of ImageJ 1.42p and before; for
     * backwards compatibility. In this mode, no checking is done whether the
     * foreground or the background gets selected; four- or 8-connected
     * behaviour depends on foreground/background and (if no selection) on
     * whether the initial pixel is on a 1-pixel wide line.
     */
    ...
    /**
     * The x-coordinates of the points in the outline. A vertical boundary at x
     * separates the pixels at x-1 and x.
     */
    public int[] xpoints = new int[maxPoints];
    /**
     * The y-coordinates of the points in the outline. A horizontal boundary at
     * y separates the pixels at y-1 and y.
     */
    ...
    /**
     * Traces an object defined by lower and upper threshold values. 'mode' can
     * be FOUR_CONNECTED or EIGHT_CONNECTED. ('LEGACY_MODE' is also supported
     * and may result in selection of interior holes instead of the thresholded
     * area if one clicks left of an interior hole). The start coordinates must
     * be inside the area or left of it. When successful, npoints>0 and the
     * boundary points can be accessed in the public xpoints and ypoints fields.
     */
     ...
     * interior hole; whatever is found first ('legacy mode'). For compatibility
     * with previous versions of ImageJ. The start coordinates must be inside
     * the area or left of it. When successful, npoints>0 and the boundary
    ...
    /**
     * This is a variation of legacy autoOutline that uses int threshold
     * arguments.
     */
    ...
    /**
     * Traces the boundary of an area of uniform color, where 'startX' and
     * 'startY' are somewhere inside the area. When successful, npoints>0 and
     * the boundary points can be accessed in the public xpoints and ypoints
     * fields. For compatibility with previous versions of ImageJ only;
     * otherwise use the reliable method specifying 4-connected or 8-connected
     * mode and the tolerance.
     */
     ...
     * uncalibrated units. 'mode' can be FOUR_CONNECTED or EIGHT_CONNECTED. Mode
     * LEGACY_MODE is for compatibility with previous versions of ImageJ;
     * ignored if tolerance > 0. Mode bit THRESHOLDED_MODE for internal use
     * only; it is set by autoOutline with 'upper' and 'lower' arguments. When
     * successful, npoints>0 and the boundary points can be accessed in the
     * public xpoints and ypoints fields.
     */
