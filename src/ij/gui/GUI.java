...
public class GUI {
     ...
     * @param accountForInsets Deduct the space taken up by menu and status
     * bars, etc. (after point is found to be inside bonds)
     * @return Rectangle of bounds or <code>null</code> if point not inside of
     * any screen.
     */
     ...
     * <code>null</code> is provided, the default screen is used.
     * @param accountForInsets Deduct the space taken up by menu and status
     * bars, etc.
     ...
     *
     * @param component the AWT component to be scaled. If a container, scaling
     * is applied to all its child components
     */
    ...
    /**
     * Tries to detect if a Swing component is unscaled and scales it it
     * according to {@link #getGuiScale()}.
     * <p>
     * This is mainly relevant to linux: Swing components scale automatically on
     * most platforms, specially since Java 8. However there are still
     * exceptions to this on linux: e.g., In Ubuntu, Swing components do scale,
     * but only under the GTK L&F. (On the other hand AWT components do not
     * scale <i>at all</i> on hiDPI screens on linux).
     * </p>
     * <p>
     * This method tries to avoid exaggerated font sizes by detecting if a
     * component has been already scaled by the UIManager, applying only
    ...
    /**
     * Works around an OpenJDK bug on Windows that causes the scrollbar thumb
     * color and background color to be almost identical.
     */
    ...
    /**
     * Returns a new NonBlockingGenericDialog with the given title, except when
     * Java is running in headless mode, in which case a GenericDialog is be
     * returned.
     */
     ...
     * Prefs.nonBlockingFilterDialogs is 'true' and 'imp' is displayed,
     * otherwise returns a GenericDialog.
     *
