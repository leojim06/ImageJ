...
public class GenericDialog extends Dialog implements ActionListener, TextListener,
    ...
    /**
     * Saves the label for given component, for macro recording and for
     * accessing the component in macros.
     */
     ...
     * additional items may be added to this line with addToSameRow()
     */
     ...
     * determined by the length of 'defaultPath', with a minimum of 25 columns.
     * Use getNextString to retrieve the directory path. Based on the
     * addDirectoryField() method in Fiji's GenericDialogPlus class.
     */
     ...
     * determined by the length of 'defaultPath', with a minimum of 25 columns.
     * Use getNextString to retrieve the file path. Based on the addFileField()
    ...
    /**
     * Adds a popup menu that lists the currently open images. Call
     * getNextImage() to retrieve the selected image. Based on the
     * addImageChoice() method in Fiji's GenericDialogPlus class.
     *
     * @param label	the label
    ...
    /**
     * Returns the selected item in the next enum choice menu. Note that
     * 'enumClass' is required to infer the proper enum type. Throws
    ...
    /**
     * Adds one or two (side by side) text areas. Append
     * "SCROLLBARS_VERTICAL_ONLY" to the text of the first text area to get
     * vertical scrollbars and "SCROLLBARS_BOTH" to get both vertical and
    ...
    /**
     * Adds a slider (scroll bar) to the dialog box. Floating point values are
     * used if (maxValue-minValue)<=5.0 and either defaultValue or minValue are
     * non-integer. @param label	the label @param minValue the minimum v
     *
     * a
     * lue of the slider @param maxValue the maximum value of the slider @param
     ...
     * component added to the dialog (except components added to the same row
     * with addToSameRow)
     * <pre>
     * Default insets:
    ...
    /**
     * Makes the next item appear in the same row as the previous. May be used
     * for addNumericField, addSlider, addChoice, addCheckbox, addStringField,
     * addMessage, addPanel, and before the showDialog() method (in the latter
     * case, the buttons appear to the right of the previous item). Note that
     * addMessage (and addStringField, if its column width is more than 8) use
    ...
    /**
     * Returns references to the "OK" ("Yes"), "Cancel", "No", and "Help"
     * buttons as an array of length 4. If a button is not present, the
    ...
    /**
     * Returns a reference to the label of the most recently added numeric
     * field, string field, choice or slider.
     */
