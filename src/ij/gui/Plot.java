 ...
 * vector fields (arrows) can be drawn on and displayed.
 *
 * Note that the clone() operation is a shallow clone: objects like arrays, the
 * PlotProperties, PlotObjects, the ImagePlus etc. of the clone remain the same
...
public class Plot implements Cloneable {
    ...
    /**
     * Masks out bits for legend positions; if all these bits are off, the
    ...
    /**
     * Draw black lines between the dots and a circle with the given color at
     * each dot
     */
    ...
    /**
     * Draw a histogram bar for each point (bars touch each other unless the x
     * axis has categories set via the axis label. x values should be sorted
     * (ascending or descending)
     */
    public static final int BAR = 11;
    /**
     * Draw a free-standing bar for each point. x values should be equidistant
     * and sorted (ascending or descending)
     */
    ...
    public static final int DEFAULT_FLAGS = X_NUMBERS + Y_NUMBERS
            + /*X_TICKS + Y_TICKS +*/ X_GRID + Y_GRID + X_LOG_TICKS + Y_LOG_TICKS;
    ...
    /**
     * Flag for copying PlotObjects (curves...) from a template if the template
     * has more PlotObjects than the Plot to copy to.
     */
    public static final int COPY_EXTRA_OBJECTS = 0x200;

    /**
     * The default margin width left of the plot frame (enough for 5-digit
     * numbers such as unscaled 16-bit
     *
     ...
     * Otherwise, must be 1.0. (creating margins, saving PlotProperties etc only
     * supports scale=1.0)
     */
    float scale = 1.0f;
    ...
    /**
     * Constructs a new Plot with the default options. Use
     * add(shape,xvalues,yvalues) to add curves.
     *
     * @param title the window title
     * @param xLabel	the x-axis label; see setXYLabels for seting categories on
     * an axis via the label
     * @param yLabel	the y-axis label; see setXYLabels for seting categories on
     * an axis via the label
    ...
    /**
     * This version of the constructor has a 'flags' argument for controlling
     * whether ticks, grid, etc. are present and whether the axes are
     * logarithmic
     */
    ...
    /**
     * Obsolete, replaced by "new Plot(title,xLabel,yLabel,flags);
     * add(shape,x,y);".
     *
     * @deprecated
     */
     ...
     * ImagePlus is non-null, its title and ImageProcessor are used, but the
     * image displayed is not modified.
     */
     ...
     * PlotProperties and (2) the serialized Vector of all 'added' PlotObjects.
     * The stream is NOT closed. The plot should have been drawn already.
     */
     ...
     * PlotProperties and (2) the serialized Vector of all 'added' PlotObjects.
     * The plot should have been drawn already. Returns null on error (which
     * should never happen).
     */
    ...
    /**
     * Returns the title of the image showing the plot (if any) or title of the
     * plot
     */
    ...
    /**
     * Sets the x-axis and y-axis range. Saves the new limits as default (so the
     * 'R' field sets the limits to these). Updates the image if existing.
    ...
    /**
     * Sets the x-axis and y-axis range. Accepts NaN values to indicate auto
     * range. Does not update the image and leaves the default limits (for reset
    ...
    /**
     * Takes over the current limits as the default ones (i.e., the limits set
     * by clicking at the 'R' field in the bottom-left corner)
     */
    ...
    /**
     * Returns the current limits as an array xMin, xMax, yMin, yMax. (note that
     * ImageJ versions before to 1.52i have returned incorrect values in case of
     * log axes) Note that future versions might return a longer array (e.g. for
     * y2 axis limits)
     */
    ...
    /**
     * Sets the current limits from an array xMin, xMax, yMin, yMax The array
     * may be also longer or shorter, but should not contain NaN values. This
     * method should be used after the plot has been displayed. Does not update
     * the plot; use updateImage() thereafter. Does not save the previous
     * limits, i.e., leaves the default limits (for reset via the 'R' field)
     * untouched.
     */
     ...
     * whitespace or commas. Note that whitespace surrounding the '=' characters
     * is not allowed. Currently recognized options are: "addhspace=10
     * addvspace=5" Increases the left&right or top&bottom margins by the given
     * number of pixels. "xinterval=30 yinterval=90" Sets interval between
     * numbers, major ticks & grid lines (default intervals are used if the
     * custom intervals would be too dense or too sparse) "xdecimals=2
     * ydecimals=-1" Sets the minimum number of decimals; use negative numbers
     * for scientific notation. "msymbol=' \\u00d7 '" Sets multiplication symbol
     * for scientific notation, here a cross with spaces.
     *
     */
    ...
    /**
     * Sets the canvas size in (unscaled) pixels and sets the scale to 1.0. If
     * the scale remains 1.0, this will be the size of the resulting
     * ImageProcessor. When not called, the canvas size is adjusted for the plot
     * size specified by setFrameSize() or setWindowSize(), or otherwise in
     * Edit>Options>Plots.
     *
    ...
    /**
     * Sets the plot frame size in (unscaled) pixels. This size does not include
     * the borders with the axis labels. Also sets the scale to 1.0. This frame
     * size in pixels divided by the data range defines the image scale. This
     * method does not check for the minimum size MIN_FRAMEWIDTH,
     * MIN_FRAMEHEIGHT. Note that the black frame will have an outer size that
     * is one pixel larger (when plotted with a linewidth of one pixel).
     *
     * @see #setWindowSize(int,int)
     */
     ...
     * (usually default) templateFlags of this plot. <code>plot</code> may be
     * null; then the call has no effect.
     */
    ...
    /**
     * Adjusts the format (style) with another plot as a template. Flags
     * determine what to copy from the template; these can be X_RANGE, Y_RANGE,
     * COPY_SIZE, COPY_LABELS, COPY_AXIS_STYLE, COPY_CONTENTS_STYLE (hidden
     * items are ignored), and COPY_LEGEND. <code>plot</code> may be null; then
    ...
    /**
     * Sets the scale. Everything, including labels, line thicknesses, etc will
     * be scaled by this factor. Also multiplies the plot size by this value.
     * Used for 'Create high-resolution plot'. Should be called before creating
     * the plot. Note that plots with a scale different from 1.0 must not be
     * shown in a PlotWindow, but only as simple image in a normal ImageWindow.
     */
    ...
    /**
     * Sets the labels of the x and y axes. 'xLabel', 'yLabel' may be null. If a
     * label has the form {txt1,txt2,txt3}, the corresponding axis will be
     * labeled not by numbers but rather with the texts "txt1", "txt2" ...
     * instead of 0, 1, ... In this special case, there will be no label for the
     * axis on the plot. Call update() thereafter to make the change visible (if
     * it is shown already).
     */
    ...
    /**
     * Sets the maximum number of intervals in a plot. Call update() thereafter
     * to make the change visible (if the image is shown already).
     */
    ...
    /**
     * Sets the length of the major tick in pixels. Call update() thereafter to
     * make the change visible (if the image is shown already).
     */
    ...
    /**
     * Sets the flags that control the axes format. Does not modify the flags
     * for logarithmic axes on/off and the FORCE2GRID flags. Call update()
     * thereafter to make the change visible (if it is shown already).
     */
    ...
    /**
     * Sets the X Axis format to Log or Linear. Call update() thereafter to make
     * the change visible (if it is shown already).
     */
    ...
    /**
     * Sets the Y Axis format to Log or Linear. Call update() thereafter to make
     * the change visible (if it is shown already).
     */
    ...
    /**
     * Sets whether to show major ticks at the x axis. Call update() thereafter
     * to make the change visible (if the image is shown already).
     */
    ...
    /**
     * Sets whether to show major ticks at the y axis. Call update() thereafter
     * to make the change visible (if the image is shown already).
     */
    ...
    /**
     * Sets whether to show minor ticks on the x axis (if linear). Also sets
     * major ticks if true and no grid is set. Call update() thereafter to make
     * the change visible (if the image is shown already).
     */
    ...
    /**
     * Sets whether to show minor ticks on the y axis (if linear). Also sets
     * major ticks if true and no grid is set. Call update() thereafter to make
     * the change visible (if the image is shown already).
     */
    ...
    /**
     * Sets the properties of the axes. Call update() thereafter to make the
     * change visible (if the image is shown already).
     */
    ...
    /**
     * Adds a curve or set of points to this plot, where 'type' is "line",
     * "connected circle", "filled", "bar", "separated bar", "circle", "box",
     * "triangle", "diamond", "cross", "x" or "dot". Run
     * <i>Help&gt;Examples&gt;JavaScript&gt;Graph Types</i> to see examples. If
     * 'type' is in the form "code: <macroCode>", the macro given is executed to
     * draw the symbol; macro variables 'x' and 'y' are the pixel coordinates of
     * the point, 'xval' and 'yval' are the plot data and 'i' is the index of
     * the data point (starting with 0 for the first point in the array). The
     * drawing including line thickness, font size, etc. be scaled by scale
     * factor 's' (to make high-resolution plots work). Example: "code:
     * setFont('sanserif',12*s,'bold
     * anti');drawString(d2s(yval,1),x-14*s,y-4*s);" writes the y value for each
    ...
    /**
     * Replaces the specified plot object (curve or set of points). Equivalent
     * to add() if there are no plot objects.
     */
     ...
     * "box", "triangle", "diamond", "cross", "x", "dot", "error bars" or
     * "xerror bars".
     */
     ...
     *
     * @param xValues	the x coordinates, or null. If null, integers starting at
     * 0 will be used for x.
     ...
     * CONNECTED_CIRCLES
     * @param label	Label for this curve or set of points, used for a legend and
     * for listing the plots
     */
    ...
    /**
     * Returns the number for a given plot symbol shape, -1 for xError and -2
     * for yError (all case-insensitive)
     */
    ...
    /**
     * Adds a set of points to the plot using double ArrayLists. Must be called
    ...
    /**
     * Adds a set of vectors to the plot using double ArrayLists. Does not
     * support logarithmic axes. Must be called before the plot is displayed.
     */
    ...
    /**
     * Draws text at the specified location, where (0,0) is the upper left
     * corner of the the plot frame and (1,1) is the lower right corner. Uses
     * the justification specified by setJustification(). When called with the
     * same position as the previous addLabel call, the text of that previous
     * call is replaced
     */
     ...
     * tab-delimited or newline-delimited list of curve or point labels in the
     * sequence these data were added. Hidden data sets are ignored. If 'labels'
     * is null or empty, the labels of the data set previously (if any) are
     * used. To modify the legend's style, call 'setFont' and 'setLineWidth'
     * before 'addLegend'.
     */
     ...
     * tab-delimited or newline-delimited list of curve or point labels in the
     * sequence these data were added. Hidden data sets are ignored; no labels
     * (and no delimiters) should be provided for these. Apart from top to
     * bottom and bottom to top (controlled by "bottom-to-top" in the options),
     * the sequence may be altered by preceding numbers followed by double
     * underscores, such as "1__Other Data Set\t0__First Data Set" (the number
     * and double underscore won't be displayed). When this possibility is used,
     * only items with double underscores will be shown in the legend (preceding
     * double underscores without a number make the item appear in the legend
     * without altering the sequence). If 'labels' is null or empty, the labels
     * of the data set previously (if any) are used. To modify the legend's
    ...
    /**
     * Adds a legend. The legend will be always drawn last (on top of
     * everything). To modify the legend's style, call 'setFont' and
     * 'setLineWidth' before 'addLegend'
     *
     * @param labels labels of the points or curves in the sequence of the data
     * were added, tab-delimited or linefeed-delimited. The labels of the
     * datasets will be set to these values. If null or not given, the labels
     * set previously (if any) will be used. Hidden data sets are ignored; no
     * labels (and no delimiters) should be provided for these. Apart from top
     * to bottom and bottom to top (controlled by the LEGEND_BOTTOM_UP flag),
     * the sequence may be altered by preceding numbers followed by double
     * underscores, such as "1__Other Data Set\t0__First Data Set" (the number
     * and double underscore won't be displayed). When this possibility is used,
     * only items with double underscores will be shown in the legend (preceding
     ...
     * LEGEND_TRANSPARENT, and LEGEND_BOTTOM_UP if desired. Updates the image
     * (if it is shown already).
     */
    ...
    /**
     * Sets the label for the plot object nuber 'index' in the sequence they
     * were added. With index=-1, sets the label for the last object added. For
     * x/y data, the label is used for the legend and as header in
     * getResultsTableWithLabels. For Text/Label objects, it affects the label
     * shown (but the plot is not redisplayed).
     */
    ...
    /**
     * Returns an array of the available curve types ("Line", "Bar", "Circle",
     * etc).
     */
    ...
    /**
     * Sets the justification used by addLabel(), where
     * <code>justification</code> is Plot.LEFT, Plot.CENTER or Plot.RIGHT.
     * Default is LEFT.
     */
     ...
     * plot. For selecting the color of the curve passed with the constructor,
     * use <code>setColor</code> before <code>draw</code>. The frame and labels
    ...
    /**
     * Changes the drawing color for the next objects that will be added to the
     * plot. It also sets secondary color: This is the color of the line for
     * CONNECTED_CIRCLES, and the color for filling open symbols (CIRCLE, BOX,
     * TRIANGLE). Set it to null or use the one-argument call setColor(color) to
     * disable filling. For selecting the color of the curve passed with the
     * constructor, use <code>setColor</code> before <code>draw</code>. The
    ...
    /**
     * Sets the drawing colors for the next objects that will be added to the
     * plot.
     */
    ...
    /**
     * Changes the line width for the next objects that will be added to the
     * plot.
     */
    ...
    /**
     * Changes the line width for the next objects that will be added to the
     * plot. After all objects have been added, set the line width to the width
     * desired for the frame around the plot (max. 3)
     */
    ...
    /**
     * Draws a line using a normalized 0-1, 0-1 coordinate system, with (0,0) at
     * the top left and (1,1) at the lower right corner. This is the same
     * coordinate system used by addLabel(x,y,label).
     */
     ...
     * currently set font when displaying the plot determines the font of all
     * labels & numbers. After the plot has been shown, sets the font for the
     * numbers and the legend (if present). If the plot is hown already, call
     * update() thereafter to make the change visible.
     */
    ...
    /**
     * Sets the font for all following addLabel() etc. operations. The currently
     * set font when displaying the plot determines the font of all labels &
     * numbers. After the plot has been shown, sets the font for the numbers and
     * the legend (if present); use setFont(char, Font) to set these fonts
     * individually. If the plot is hown already, call update() thereafter to
     ...
     * operations. This leaves the font name and style of the previously used
     * fonts unchanged. The currently set font when displaying the plot
     * determines the font of the numbers at the axes. That font also sets the
     * default label font size, which may be overridden by setAxisLabelFontSize
     * or setXLabelFont, setYLabelFont. After the plot has been shown, sets the
     * font for the numbers and the legend (if present); use setFont(char, Font)
     * to set these fonts individually. Styles are defined in the Font class,
     * e.g. Font.PLAIN, Font.BOLD. Set <code>style</code> to -1 to leave the
     * style unchanged. Call update() thereafter to make the change visible (if
     * the image is shown already).
     */
    ...
    /**
     * Sets the x and y label font size and style. Styles are defined in the
     * Font class, e.g. Font.PLAIN, Font.BOLD. Set <code>style</code> to -1 to
     * leave the style unchanged. Call update() thereafter to make the change
    ...
    /**
     * Sets the xLabelFont; must not be null. If this method is not used, the
     * last setFont of setFontSize call before displaying the plot determines
     * the font, or if neither was called, the font size of the Plot Options is
     * used.
     */
    ...
    /**
     * Gets the font for xLabel ('x'), yLabel('y'), numbers ('f' for 'frame') or
     * the legend ('l'). Returns null if the given PlotObject does not exist or
     * its font is null
     */
    ...
    /**
     * Sets the font for xLabel ('x'), yLabel('y'), numbers ('f' for 'frame') or
     * the legend ('l')
     */
    ...
    /**
     * Gets the label String of the xLabel ('x'), yLabel('y') or the legend
     * ('l'). Returns null if the given PlotObject does not exist or its label
     * is null
     */
     ...
     * null) or otherwise of the data set of the first 'addPoints'. Returns null
     * if neither exists
     */
    ...
    /**
     * Get the data of the n-th Plot Object containing xy data in the sequence
     * they were added (Other Plot Objects such as labels, arrows, lines, shapes
     * and hidden PlotObjects are not counted). The array returned has elements
     * [0] x data, [1] y data, [2] x error bars, [3] y error bars. If no error
     * bars are given, the corresponding arrays are null. Returns null if there
     * is no Plot Object with xy data with this index.
     *
    ...
    /**
     * Gets an array with human-readable designations of the PlotObjects
     * (curves, labels, ...) in the sequence they were added (the object passed
     * with the constructor is first, even though it is plotted last). Hidden
     * PlotObjects are included. *
     */
    ...
    /**
     * Gets an array with human-readable designations of the PlotObjects
     * containing xy data in the sequence they were added. Other Plot Objects
     * such as labels, arrows, lines, shapes and hidden PlotObjects are not
     * counted. (the object passed with the constructor is first, even though it
     * is plotted last).
     */
     ...
     * constructor or added by 'add' or 'draw' methods. Legend, frame and axes
     * (though internally PlotObjects) are not included
     */
    ...
    /**
     * Returns the number of PlotObjects fitting the mask. Legend, frame and
     * axes (though internally PlotObjects) are not included
     */
    ...
    /**
     * Gets an array with human-readable designations of the PlotObjects with
     * types fitting the mask (i.e., 'mask' should be a bitwise or of the types
     * desired)
     */
     ...
     * hidden ones) from another plot to this one. PlotObjects here refers to
     * curves, arrows, labels etc. (not legend, axes and frame, though
     * implemented as PlotObjects) Use 'update' to update the plot thereafter.
     *
     ...
     * XY_DATA] [,hidden] PlotObjects here refers to curves, arrows, labels etc.
     ...
     * including hidden ones). Returns null if no label. PlotObjects here refers
     * to curves, arrows, labels etc. (not legend, exes and frame, though
     * implemented as PlotObjects)
     */
     ...
     * including hidden ones) PlotObjects here refers to curves, arrows, labels
     * etc. (not legend, exes and frame, though implemented as PlotObjects)
     */
     ...
     * PlotObjects here refers to curves, arrows, labels etc. (not legend, exes
     * and frame, though implemented as PlotObjects)
     */
    ...
    /**
     * Returns the index of the first plot object with x,y data (points, line)
     * or arrows with all data equal to those given. Returns or -1 is no such
     * plot object exists. The array 'values' should contain the x, y, x error
     * bar, yerror bar data. The 'values' array may have any size; only the data
     * given are compared (e.g. for an array with length 2, there is no check
     * for erro bars). Used when adding data from a table not to suggest the
     * same data twice.
     */
    ...
    /**
     * Returns the index of the first plot object fitting the type mask and with
     * all data equal to those given. ('mask' should be a bitwise or of the
     * types desired) Returns or -1 is no such plot object exists. The array
     * 'values' should contain the x, y, x error bar, yerror bar data. The
     * 'values' array may have any size; only the data given are compared (e.g.
     * for an array with length 2, there is no check for erro bars). Used when
     * adding data from a table not to suggest the same data twice.
     */
    ...
    /**
     * Restores the plot contents (not including axis formats etc) from the
     * snapshot previously created by savePlotObjects(). See also
     * killPlotObjectsSnapshot Use 'update' to update the plot thereafter.
     */
     ...
     * PlotObjects such as data and corresponding curves etc.), for later undo
     * by restorePlotProperties. See also killPlotPropertiesSnapshot
     */
    ...
    /**
     * Restores the plot properties (formatting, range etc., not PlotObjects
     * such as data and corresponding curves etc.) from a snapshot previously
     * created by savePlotPlotProperties. See also killPlotPropertiesSnapshot.
     ...
     * given by setLimits. Updates the image if existing and updateImg is true
     */
    ...
    /**
     * Returns the plot as an ImagePlus. If an ImagePlus for this plot already
     * exists, displays the plot in that ImagePlus and returns it.
     */
    ...
    /**
     * Sets the ImagePlus where the plot will be displayed. If the ImagePlus is
     * not known otherwise (e.g. from getImagePlus), this is needed for changes
     * such as zooming in to work correctly. It also sets the calibration of the
     * ImagePlus. The ImagePlus is not displayed or updated unless its
     * ImageProcessor is no that of the current Plot (then it gets this
     * ImageProcessor). Does nothing if imp is unchanged and has the
     * ImageProcessor of this plot. 'imp' may be null to disconnect the plot
     * from its ImagePlus. Does nothing for Plot Stacks.
     */
    ...
    /**
     * Adjusts a Calibration object to fit the current axes. For log axes, the
     * calibration refers to the base-10 logarithm of the value
     */
    ...
    /**
     * Displays the plot in a PlotWindow. Plot stacks are shown in a
     * StackWindow, not in a PlotWindow; in this case the return value is null
     * (use getImagePlus().getWindow() instead). Also returns null in BatchMode.
     * Note that the PlotWindow might get closed immediately if its 'listValues'
     * and 'autoClose' flags are set.
     *
     * @see #update()
     */
    ...
    /**
     * Appends the current plot to a virtual stack and resets allPlotObjects for
     * next slice N. Vischer
     */
     ...
     * been drawn already. Call getProcessor to retrieve the ImageProcessor with
     * it. Does no action with respect to the ImagePlus (if any)
     */
    public void draw() {
     ...
     * resized or updated, and the Plot class does no modifications to the
     * ImageProcessor. Changes are recorded nevertheless and become effective
     * with <code>setFrozen(false)</code>.
     */
    ...
    /**
     * Draws the plot again, ignored if the plot has not been drawn before or
     * the plot is frozen.
     */
    public void update() {
        updateImage();
    ...
    /**
     * Draws the plot again, ignored if the plot has not been drawn before or
     * the plot is frozen. If the ImagePlus exist, updates it and its
     * calibration.
     */
    ...
    /**
     * Returns the rectangle where the data are plotted. This rectangle includes
     * the black outline frame at the top and left, but not at the bottom and
     * right (when the frame is plotted with 1 pxl width). The image scale is
     * its width or height in pixels divided by the data range in x or y.
     */
    ...
    /**
     * Creates a new high-resolution plot by scaling it and displays that plot
     * if showIt is true. <code>title</code> may be null, then a default title
     * is used.
     */
    ...
    /**
     * Releases the ImageProcessor and ImagePlus associated with the plot. May
     * help garbage collection because some garbage collectors are said to be
     * inefficient with circular references.
     */
    public void dispose() {
        if (imp != null) {
     ...
     * calibration, also works with log axes and inverted x axes
     */
    ...
    /**
     * Converts pixels to calibrated coordinates. In contrast to the image
     * calibration, also works with log axes
     */
     ...
     * image calibration, also works with log x axis and inverted x axis
     */
    ...
    /**
     * Converts calibrated coordinates to pixel coordinates. In contrast to the
     * image calibration, also works with log y axis
     */
    ...
    /**
     * Converts calibrated coordinates to pixel coordinates. In contrast to the
     * image calibration, also works with log axes
     */
    ...
    /**
     * Converts calibrated coordinates to pixel coordinates. In contrast to the
     * image calibration, also works with log axes and inverted x axes. Returns
     * a large number instead NaN for log x axis and zero or negative x
     */
     ...
     * Converts calibrated coordinates to pixel coordinates. In contrast to the
     * image calibration, also works with log axes and inverted x axes. Returns
     * a large number instead NaN for log y axis and zero or negative y
     */
    ...
    /**
     * Scales a value of the original plot for a high-resolution plot. Returns
     * an integer number of pixels >=1
     */
    int sc(float length) {
    ...
    /**
     * Whether a color is non-grayscale, which requires color (not grayscale)
     * for the plot
     */
    ...
    /**
     * Draws the plot contents (all PlotObjects and the frame and legend),
     * without axes etc.
     */
     ...
     * it for plotting. Also called by the PlotWindow class to prepare the
     * window.
     */
    ...
    /**
     * Calculate the actual range, major step interval and set variables for
     * data <-> pixels scaling
     */
     ...
     * previous versions of ImageJ, only the first PlotObject (with numeric
     * data) is used to determine the limits.
     */
     ...
     * plotObject or all such plotObjects; axisRangeFlags determine for which
     * axis to calculate the min&max (X_RANGE for x axis, Y_RANGE for y axis);
     * for the other axes the limit is taken from defaultMinMax Array elements
     * returned are xMin, xMax, yMin, yMax. Also sets enlargeRange to tell which
     * limits should be enlarged beyond the minimum or maximum of the data
     */
    ...
    /**
     * Enlarges the current minimum and maximum ranges to include the data range
     * of the last plotObject added, if it is an XY_DATA or ARROWS plotObject.
     ...
     * axisRangeFlags determine for which axis (X_RANGE for x axis, Y_RANGE for
     * y axis) The minimum modifies allMinAndMax[0] (x), allMinAndMax[2] (y);
     * the maximum modifies [1], [3]. If allMinAndMax values are modified, the
     * corresponding enlargeRange array elements are also set
     */
    ...
    /**
     * Gets the minimum and maximum values for a dataset (one direction, x or
     * y), taking error bars (if not null) into account. The minimum modifies
     * allMinAndMax[axisIndex] the maximum modifies allMinAndMax[axisIndex+1].
     * Also cares about whether the range should be enlarged to avoid hiding
     * markers at the borders: suggestedEnlarge is 0 for lines or a suggestion
     * for the data type; if the allMinAndMax is range is extended, the
     * corresponding enlargeRange item is set accordingly
     */
    ...
    /**
     * Modifies automatic limits to avoid points exactly at the borders. This
     * only happens for limits where the corresponding value of the class array
     * 'enlargeRange' is set: ALWAYS_ENLARGE: always enlarges the range;
     * USUALLY_ENLARGE means that the limits should not be shifted across zero
     */
    ...
    /**
     * Returns the first font of the list that is not null, or defaultFont if
     * both are null
     */
    ...
    /**
     * Adjusts the plot range when the user clicks one of the overlay symbols at
     * the axes. Index numbers for arrows start with 0 at the 'down' arrow of
     * the lower side of the x axis and end with 7 the up arrow at the upper
     ...
     * options dialog. Numbers 10-15 must correspond to the dialogTypes as
     * defined in PlotDialog.
     */
    ...
    /**
     * Zooms in or out active plots while keeping focus on cursor position Above
     * or below frame: zoom x only Left or right of frame: zoom y only Corners:
     * focus is in center N. Vischer
     */
    ...
    /**
     * Whether to draw simple axes without ticks, grid and numbers only for min,
     * max
     */
    ...
    /**
     * Draws ticks, grid and axis label for each tick/grid line. The grid or
     * major tick spacing in each direction is given by steps
     */
    ...
    /**
     * Returns the smallest "nice" number >= v. "Nice" numbers are .. 0.5, 1, 2,
     * 5, 10, 20 ...
     */
     ...
     * negative to draw only 10^exponent Currently only supports center
     * justification and right justification (y of center line) Fonts baseFont,
     * smallFont should be scaled already Returns the width of the String
     */
    ...
    /**
     * Returns the user-supplied (via setOptions) or default multiplication
     * symbol (middot)
     */
     ...
     * 'resolution'; (if n is integer and small enough to display without
     * scientific notation, no decimals are needed, irrespective of
     * 'resolution') Scientific notation is used for more than 'maxDigits' (must
     * be >=3), and indicated by a negative return value, or if suggestedDigits
     * is negative Returns 'suggestedDigits' if not 0 and compatible with the
     * resolution; negative values of 'suggestedDigits' switch to scientific
     * notation.
     */
     ...
     * 'resolution'; Scientific notation is used for more than 'maxDigits' (must
     * be >=3), and indicated by a negative return value Returns
     * 'suggestedDigits' if not 0 and compatible with the resolution; negative
     * values of 'suggestedDigits' switch to sceintific notation.
     */
    ...
    void drawBarChart(PlotObject plotObject) {
        ...
        if (separatedBars && n > 1) {
            halfBarWidthInPixels = Math.max(1, (int) Math.round(Math.abs(0.5 * (plotObject.xValues[n - 1] - plotObject.xValues[0]) / (n - 1) * xScale * SEPARATED_BAR_WIDTH)));
        }
    ...
    /**
     * Draw the symbol for the data point number 'pointIndex' (pointIndex < 0
    ...
    /**
     * Fill the area of the symbols for data points (except for shape=DOT) Note
     * that ip.fill, ip.fillOval etc. can't be used here: they do not care about
     * the clip rectangle
     */
    ...
    /**
     * Adds an arrow from position 1 to 2 given in pixels; 'size' is the length
     * of the arrowhead
     *
     * @deprecated Use as a public method is not supported any more because it
     * is incompatible with rescaling
     */
    @Deprecated
    ...
    /**
     * Fills space between polyline and y=0 with the current color (the
     * secondary color of the plotObject)
     */
    ...
    /**
     * Returns the x, y coordinates at the cursor position or the nearest point
     * as a String
     */
     ...
     * constructor or (if that was null) the first x & y data added later.
    ...
    /**
     * Returns a reference to the PlotObject with x & y data (points, curve)
     * added last, or null if none.
     */
    ...
    /**
     * returns whether there are several plots so that one cannot give a single
    ...
    /**
     * Returns the labels of the (non-hidden) datasets as linefeed-delimited
     * String. If the label is not set, a blank line is added.
     */
    ...
    /**
     * Creates a ResultsTable with the plot data. Returns an empty table if no
     * data.
     */
    ...
    /**
     * Creates a ResultsTable with the data of the plot. Returns null if no
     * data. Does not write the first x column if writeFirstXColumn is false.
     * When all columns are the same length, x columns equal to the first x
     * column are not written, independent of writeFirstXColumn. Column headings
     * are "X", "Y", "X1", "Y1", etc, irrespective of any labels of the data
     * sets
     */
    ...
    /**
     * Creates a ResultsTable with the data of the plot. Returns null if no
     * data. When all columns are the same length, x columns equal to the first
     * x column are not written, independent of writeFirstXColumn. When the data
     * sets have labels, they are used for column headings
     */
     ...
     * data. Does not write the first x column if writeFirstXColumn is false.
     * When all columns are the same length, x columns equal to the first x
     * column are not written, independent of writeFirstXColumn. When the data
     * sets have labels and useLabels is true, they are used for column
     * headings, otherwise columns are named X, Y, X1, Y1, ...
     */
    ...
    /**
     * Convert a string to a label suitable for a ResultsTable without
     * whitespace, quotes or commas, to avoid problems when saving and reading
     * the table. Returns null if an empty string or null.
     */
     ...
     *
     * @param values	array containing the population N.Vischer
     */
     ...
     * @param binWidth	set zero for auto-binning N.Vischer
     */
     ...
     * @param binCenter any x value can be the center of a bin N.Vischer
     */
 ...
 * etc, except for the data+format (plot contents). To enable reading serialized
 * PlotObjects of plots created with previous versions of ImageJ, the variable
 * names MUST NEVER be changed! Also any additions should be made after careful
 * thought, since they have to be kept for all future versions.
 */
class PlotProperties implements Cloneable, Serializable {

    /**
     * The serialVersionUID should not be modified, otherwise saved plots won't
     * be readable any more
     */
    ...
    /**
     * Returns an array of all PlotObjects defined as PlotProperties. Note that
     * some may be null
     */
    ...
    /**
     * Returns the PlotObject for xLabel ('x'), yLabel('y'), frame ('f';
     * includes number font) or the legend ('l').
     */
 ...
 * arrows, a line or a label in a plot, as well as the legend, axis labels, and
 * frame (including background and fonts of axis numbering). Note that all
 * properties such as lineWidths and Fonts have to be scaled up for
 * high-resolution plots. This class allows serialization for writing into tiff
 * files. To enable reading serialized PlotObjects of plots created with
 * previous versions of ImageJ, the variable names MUST NEVER be changed! Also
 * any additions should be made after careful thought, since they have to be
 * kept for all future versions.
 */
class PlotObject implements Cloneable, Serializable {
    ...
    /**
     * flag for the data set passed with the constructor. Note that 0 to 0x0f
     * are reserved for fonts modifiers, 0x010-0x800 are reserved for legend
     * modifiers
     */
    ...
    /**
     * Type of the object; XY_DATA stands for curve or markers, can be also
     * ARROWS ... SHAPES
     */
    ...
    /**
     * The x and y data arrays and the error bars (if non-null). These arrays
     * also serve as x0, y0, x1, y1 arrays for plotting arrays of arrows
     */
    public float[] xValues, yValues, xEValues, yEValues;
    /**
     * For SHAPES: For boxplots with whiskers ('boxes'), elements of the
     * ArrayList are float[6] with x and all 5 y values (for 'boxesx', y and all
     * 5 x values), for 'rectangles', float[4] with x1, y1, x2, y2.
     */
    ...
    /**
     * Type of the points, such as Plot.LINE, Plot.CROSS etc. (for type =
     * XY_DATA)
     */
    public int shape;
    /**
     * The line width in pixels for 'normal' plots (for high-resolution plots,
    ...
    public int step;
    /**
     * A label for the y data of the curve, a text to draw, or the text of a
     * legend (tab-delimited lines)
     */
    public String label;
    /**
    ...
    /**
     * String for representation of the font family (for Serialization); may be
     * null for default. Font style is in flags, font size in fontSize.
     */
    private String fontFamily;
    /**
     * Font size (for Serialization), for 'normal' plots (for high-resolution
     * plots, to be multiplied by a scale factor)
     */
    ...
    /**
     * Constructor for the legend. <code>flags</code> is bitwise or of TOP_LEFT
     * etc. and LEGEND_TRANSPARENT if desired. Note that the labels in the
     * legend are those of the data plotObjects
     */
    ...
    /**
     * Constructor for the frame, including axis numbers. In the current
     * version, the primary color (line color) is always black
     */
    ...
    /**
     * Returns all data xValues, yValues, xEValues, yEValues as a float[][]
     * array. Note that future versions may have more data.
     */
    ...
    /**
     * A deep clone, which duplicates arrays etc. Note that colors & font are
     * not cloned; it is assumed that these wil not be modified but replaced, so
     * the clone remains unaffected
     */
    ...
    /**
     * A clone of an array list one level deeper than a shallow clone. The
     * clone() method of the objects in the list must be accessible
     */
