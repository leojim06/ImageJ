...
public class PolygonRoi extends Roi {
    ...
    /**
     * Creates a new polygon or polyline ROI from float x and y arrays. Type
     * must be Roi.POLYGON, Roi.FREEROI, Roi.POLYLINE, Roi.FREELINE or
     * Roi.ANGLE.
     */
    ...
    /**
     * Fits a spline, but leaves the Roi unchanged. The arrays supplied (which
     * may be null) are reused if the number of spline points remains unchanged.
     *
     ...
     * and sets it as the Roi shape. It can be retrieved using the
     * getFloatPolygon() method.
     */
    ...
    /**
     * Interpolates an open polgonial shape to get line segments of (roughly)
     * length 'segmentLength', and returns arrays of x & y coordinates. Since
     * the full length is not necessarily an integer multiple of
     * 'segmentLength', the segmentLength actually used is returned as array
     * element [2][0]. If the ImagePlus is given, calibrated coordinates are
     * used, based on its calibration, and 'segmentLength' should be in
     * calibrated units. Returns null if an input array is null, or the number
     * of points is 0
     */
     ...
     * position of a pixel center determines whether a pixel is selected. Note
     * the ImageJ convention of 0.5 pixel shift between outline and pixel
     * centers, i.e., pixel (0,0) is enclosed by the rectangle spanned between
     * (0,0) and (1,1). Points exactly at the left (right) border are considered
     * outside (inside); points exactly on horizontal borders, are considered
     * outside (inside) at the border with the lower (higher) y. This convention
     * is opposite to that of the java.awt.Shape class. In x, the offset is
     * chosen slightly below 0.5 to reduce the impact of numerical errors.
     */
     ...
     * pixels in corners as sqrt(2). It does this by calculating the total
     * length of the ROI boundary and subtracting 2-sqrt(2) for each
     * non-adjacent corner. For example, a 1x1 pixel ROI has a boundary length
     * of 4 and 2 non-adjacent edges so the perimeter is 4-2*(2-sqrt(2)). A 2x2
     * pixel ROI has a boundary length of 8 and 4 non-adjacent edges so the
     * perimeter is 8-4*(2-sqrt(2)). Note that this code can currently create
     * inconsistent legths depending on the starting position.
     */
    ...
    /**
     * Returns the perimeter (for ROIs) or length (for lines), using calibration
     * if imp is not null
     */
    ...
    /**
     * Returns 'true' if this ROI was created using the wand tool or the
     * particle analyzer.
     */
    ...
    /**
     * Returns the length of a polygon with integer coordinates. Uses no
     * calibration if imp is null.
     */
    ...
    /**
     * Returns the length of a polygon with float coordinates. Uses no
     * calibration if imp is null.
     */
    ...
    /**
     * Returns this polygon or polyline as float arrays in image pixel
     * coordinates.
     */
