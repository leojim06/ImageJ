...
public class ThreadUtil {
 ...
 /* The following is for parallelization using a ThreadPool, which avoids the
    ...
    /**
     * The threadPoolExecutor holds at least as many threads for parallel
     * execution as the number of processors; additional threads are added as
     * required. These additional threads will be terminated if idle for 120
     * seconds.
     */
     ...
     * and waits until each of them has finished. If the current thread is
     * interrupted, each of the callables gets cancelled and interrupted. Also
     * in that case, waits until all callables have finished. The 'interrupted'
     * status of the current thread is preserved, as required for preview in an
     * ImageJ ExtendedPlugInFilter. Note that ImageJ requires that all callables
     * can run concurrently, and none of them must stay in the queue while
     * others run. (This is required by the RankFilters, where the threads are
     * not independent)
     *
     * @param callables Array of tasks. If no return value is needed, best use
     * <code>Callable<Void></code> (then the <code>Void call()</code> method
     * should return null). If the array size is 1, the <code>call()</code>
     ...
     * return results, the get() methods of these Futures may be used to get the
     * results.
     */
     ...
     *
     * @param callables Array of tasks; these might be
     * <code>Callable<Void></code> if no return value is needed (then the
     * <code>call</code> methods should return null).
     * @return Array of the <code>java.util.concurrent.Future</code>s,
     * corresponding to the callables. The futures may be used to wait for
     * completion of the callables or cancel them. If the call methods of the
     * callables return results, these Futures may be used to get the results.
     */
     ...
     * <code>Future</code>s given. If the current thread is interrupted, each of
     * the <code>Callable</code>s gets cancelled and interrupted. Also in that
     * case, this method waits until all callables have finished. The
     * 'interrupted' status of the current thread is preserved, as required for
     * preview in an ImageJ ExtendedPlugInFilter.
     */
