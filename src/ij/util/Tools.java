...
public class Tools {
    ...
    /**
     * Converts an int to a zero-padded hex string of fixed length 'digits'. If
     * the number is too high, it gets truncated, keeping only the lowest
     * 'digits' characters.
     */
    ...
    /**
     * Determines the minimum and maximum value in the array <code>a</code> and
     * returns them as 2-element array {minimum, maximum}.
     */
     ...
     * @param defaultValue the value returned if <code>s</code> does not contain
     * a parsable double
    ...
    /**
     * Returns the number of decimal places needed to display a number, or -2 if
    ...
    /**
     * Returns the number of decimal places needed to display two numbers, or -2
     * if exponential notation should be used.
     */
    ...
    /**
     * Opens a text file in ij.jar as a String (example path:
     * "/macros/Circle_Tool.txt").
     */
    ...
    /**
     * Copies the contents of the file at 'path1' to 'path2', returning an error
     * message (as a non-empty string) if there is an error. Based on the method
     * with the same name in Tobias Pietzsch's TifBenchmark class.
     */
    ...
    /**
     * Retrieves a number form a list of key-number pairs like "value1=1234.5
     * area=1.2e6". The "=" (if present) must be part of the 'key' string.
     * Delimiters may be commas, semicolons or whitespace. There must be no
     * whitespace between key and number. Returns Double.NaN if 'list' is null,
     * if the key is not found, if the number is 'NaN' or invalid.
     */
    ...
    public static double getNumberFromList(String list, String key, double defaultValue) {
        ...
        int n = start;
        while (n < list.length() && !isDelimiter(list.charAt(n))) {
            n++;
        }
    ...
    /**
     * Retrieves a String form a list of key-number pairs like "value1="abc"
     * str='the Text'". The "=" (if present) must be part of the 'key' string.
     * Strings containing commas, semicolons or whitespace must be enclosed in
     * single or double quotes. There must be no whitespace between key and
     * quotes or key and unquoted String. Strings must not contain a key. In
     * case of doubt, e.g., replace the '=' with '\=' Escape sequences with
     * backslashes are recognized, also backslash-u???? for unicode. Returns
     * null if 'list' is null or if the key is not found
     */
    ...
    /**
     * Decodes backslash-escaped characters from the String until the character
     * 'delim' is found. With 'delim'=-1, does not search for a delimiter, with
     * 'delim'=(char)-2, (unescaped) whitespace, commas and semicolons are
     * interpreted as delimiters. Also decodes unicode backslash-u????
     * characters
     */
    public static String decodeEscaped(String str, char delim) {
        ...
        for (int i = 0; i < str.length(); i++) {
            char c = str.charAt(i);
            if ((delim == (char) -2 && isDelimiter(c)) || c == delim) {
                break;
            }
    ...
    /**
     * Returns the escaped character if there was a preceding backslash. Returns
     * 'c' if there is no known escape sequence backslash-c
     */
    ...
    /**
     * Returns the checksum of a string or file, or "0" if no success. The
     * 'method' argument must be "MD5" or "SHA-256".
     */
