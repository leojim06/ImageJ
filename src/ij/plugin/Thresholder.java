...
/**
 * This plugin implements the Process/Binary/Make Binary and Convert to Mask
 * commands.
 */
public class Thresholder implements PlugIn, Measurements, ItemListener {
    ...
    private void applyThreshold(ImagePlus imp, boolean oneSlice) {
        ...
        if (IJ.debugMode) {
            IJ.log("Thresholder (apply): " + minThreshold + "-" + maxThreshold + " " + fcolor + " " + bcolor + " " + fill1 + " " + fill2);
        }
        int[] lut = new int[256];
        for (int i = 0; i < 256; i++) {
    ...
    void autoThreshold(ImageProcessor ip) {
        ...
        maxThreshold = ip.getMaxThreshold();
        if (IJ.debugMode) {
            IJ.log("Thresholder (auto): " + minThreshold + "-" + maxThreshold);
        }
    }
