 ...
 * Outliers, Remove NaNs and Despeckle commands.
 */
...
public class RankFilters implements ExtendedPlugInFilter, DialogListener {
     ...
     * Circular Masks. Must not be negative. No checking is done for large
     * values that would lead to excessive computing times.
     ...
     * use median and a radius of 1.0 instead
     * @param whichOutliers BRIGHT_OUTLIERS or DARK_OUTLIERS for 'outliers'
     * filter
     ...
     * light, not dark
     * @param dontSubtract fpr top-hat filter, performs a grayscale open or
     * close instead of top-hat, where the result of grayscale open/close is
     ...
     * outside the kernel area. x between 0 and cacheWidth-1
     */
    ...
    /**
     * Get sum of values and values squared within the kernel area. x between 0
     * and cacheWidth-1 Output is written to array sums[0] = sum; sums[1] = sum
     * of squares
     */
    ...
    /**
     * Get median of values within kernel-sized neighborhood. Kernel size
     * kNPoints should be odd.
     */
    ...
    /**
     * Get median of values within kernel-sized neighborhood. NaN data values
     * are ignored; the output is NaN only if there are only NaN values in the
    ...
    /**
     * Reset region between inner rectangle 'roi' and outer rectangle 'roi1' to
     * the snapshot
     */
    ...
    /**
     * @deprecated Not needed any more, use the rank(ip, ...) method, which
     * creates the kernel
     */
    ...
    /**
     * Sets the display range after variance and top-hat (16-bit &
     ...
     *
     * @param radius Radius = 0.5 includes the 4 neighbors of the pixel in the
     * center, radius = 1 corresponds to a 3x3 kernel size.
     * @return the circular kernel The output is an array that gives the length
     * of each line of the structuring element (kernel) to the left (negative)
     * and to the right (positive): [0] left in line 0, [1] right in line 0, [2]
     * left in line 2, ... The maximum (absolute) value should be kernelRadius.
     * Array elements at the end: length-2: nPoints, number of pixels in the
     * kernel area length-1: kernelRadius in x direction (kernel width is
     * 2*kernelRadius+1) Kernel height can be calculated as (array length - 1)/2
     * (odd number); Kernel radius in y direction is kernel height/2 (truncating
     * integer division). Note that kernel width and height are the same for the
     * circular kernels used here, but treated separately for the case of future
     * extensions with non-circular kernels.
     */
     ...
     * corresponding to 100% of the progress bar. Setting nPasses=0 suppresses
     * the progress bar
     */
