...
/**
 * This plugin implements the Euclidean Distance Map (EDM), Watershed, Ultimate
 * Eroded Points and Voronoi commands in the Process/Binary submenu.
 *
 * - Euclidean Distance Map: The value of each pixel is the distance to the
 * nearest background pixel (for background pixels, the EDM is 0) - Ultimate
 * Eroded Points (UEPs) are maxima of the EDM. In the output, the points are
 * assigned the EDM value, which is equal to the radius of the largest circle
 * that fits into the particle, with the UEP as the center. - Watershed
 * segmentation of the EDM splits particles at "necks"; starting at maxima of
 * the EDM. - 'Voronoi' splits the image by lines of points having equal
 * distance to the borders of the two nearest particles. Thus, the Voronoi cell
 * of each particle includes all points that are nearer to this particle than
 * any other particle. For the case of the priticles being single points, this
 * is a Voronoi tessellation (also known as Dirichlet tessellation). In the
 * output, the value inside the Voronoi cells is zero; the pixel values of the
 * dividing lines between the cells are equal to the distance to the two nearest
 * particles. This is similar to a medial axis transform of the background, but
 * there are no lines in inner holes of particles.
 *
 * Watershed, Ultimate Eroded Points and Voronoi are handled by the
 * MaximumFinder plugin applied to the EDM Note: These functions do not take
 * ROIs into account. Setup is called with argument "" (empty string) for EDM,
 * "watershed" for watershed segmentation, "points" for ultimate eroded points
 * and "voronoi" for Voronoi segmentation of the background
 *
 * The EDM algorithm is similar to the 8SSEDT in F. Leymarie, M. D. Levine, in:
 * CVGIP Image Understanding, vol. 55 (1992), pp 84-94
 * http://dx.doi.org/10.1016/1049-9660(92)90008-Q
 *
 * The algorithm provides a fast approximation of the EDM, with the deviation
 * from a full calculation being between -0.09 and 0. The algorithm is exact for distances<13.
 * For d>=13, deviations from the true result can occur, but are very rare:
 * typically the fraction of pixels deviating from the exact result is in the
 * 10^-5 range, with most deviations between -0.03 and -0.04.
 *
 * Limitations: Maximum image diagonal for EDM: 46340 pixels (sqrt(2^31)); if
 * the particles are dense enough it also works for width, height <=65534.
 *
 * Version 30-Apr-2008 Michael Schmid: more accurate EDM algorithm, 16-bit and
...
public class EDM implements ExtendedPlugInFilter {
    ...
    /**
     * Unit in old make16bitEDM: this pixel value corresponds to a distance of
     * one pixel. For compatibility only.
     */
     ...
     * (EDM). Each foreground (nonzero) pixel in the binary image is assigned a
     ...
     * (nonzero pixels) in an 8-bit image. Particles are segmented by their
     * shape; segmentation lines added are background pixels (value = 0);
     */
     ...
     * value equal to its distance from the nearest background (zero) pixel,
     * multiplied by EDM.ONE. For compatibility with previous versions of ImageJ
     * only.
     */
     ...
     * @param backgroundValue Pixels in the input with this value are
     * interpreted as background. Note: for pixel value 255, write either -1 or
     * (byte)255.
     * @param edgesAreBackground Whether out-of-image pixels are considered
     * background
     * @return The EDM, containing the distances to the nearest background
