...
public class GaussianBlur implements ExtendedPlugInFilter, DialogListener {
    ...
    /**
     * whether sigma is given in units corresponding to the pixel scale (not
     * pixels)
     */
     ...
     * images to float as well as the two filter directions are taken into
     * account. Otherwise, the caller should set nPasses to the number of
     * 1-dimensional filter operations required.
     */
     ...
     *
     * @param ip The image subject to filtering. It must have a valid snapshot
     * if the height of the roi is less than the full image height.
     */
     ...
     * with the previous code (before 1.38r) and uses a low-accuracy kernel,
     * only slightly better than the previous ImageJ code. The 'radius' in this
     * call is different from the one used in ImageJ 1.38r and later. Therefore,
     * use blurGaussian(ip, sigma, sigma, accuracy), where 'sigma' is equivalent
     * to the 'sigma (radius)' of the Menu, and accuracy should be 0.02 unless
     * better accuracy is desired.
     */
    @Deprecated
    ...
    /**
     * Blur an image in one direction (x or y) by a Gaussian, using multiple
     * threads on multiprocessor machines
     *
