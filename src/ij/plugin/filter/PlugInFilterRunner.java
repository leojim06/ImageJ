...
public class PlugInFilterRunner implements Runnable, DialogListener {
    ...
    /**
     * The constructor runs a PlugInFilter or ExtendedPlugInFilter by calling
     * its setup, run, etc. methods. For details, see the documentation of
     ...
     * or in the plugins.config file of a .jar archive conatining a collection
     * of plugins. <code>arg</code> may be a string of length zero.
     */
     ...
     * @param fp	A Floatprocessor as a target for conversion to Float. May be
     * null.
     * @param snapshotPixels Valid snapshotPixels for the current ImageProcessor
     * (null when processing a stack) Class variables used: flags (input),
     * snapshotPixels (set if a snapshot of ip is taken), ipChanged (set if ip
    ...
    /**
     * Make the slice number accessible to the PlugInFilter by putting it into
     * the appropriate hashtable.
     */
     ...
     * invoked every time the user changes something in the dialog (except OK
     * and cancel buttons), provided that all previous listeners (parameter
     * checking) have returned true.
     *
