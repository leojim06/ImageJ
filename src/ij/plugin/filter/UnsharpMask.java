...
/**
 * This plugin-filter implements ImageJ's Unsharp Mask command. Unsharp masking
 * subtracts a blurred copy of the image and rescales the image to obtain the
 * same contrast of large (low-frequency) structures as in the input image. This
 * is equivalent to adding a high-pass filtered image and thus sharpens the
 * image. "Radius (Sigma)" is the standard deviation (blur radius) of the
 * Gaussian blur that is subtracted. "Mask Weight" determines the strength of
 * filtering, where "Mask Weight"=1 would be an infinite weight of the high-pass
...
public class UnsharpMask implements ExtendedPlugInFilter, DialogListener {
    ...
    /**
     * This method is invoked for each slice or color channel. It filters an
     * image by enhancing high-frequency components. Since this PlugInFilter
     * specifies the CONVERT_TO_FLOAT and SNAPHOT flags, 'ip' is always a
     * FloatProcessor with a valid snapshot.
     *
     ...
     * information about the number of passes to Gaussian Blur. The ProgressBar
     * will be handled by GaussianBlur.
     */
