 ...
 * Uncompressed 8-bit (gray or indexed color), 24-bit (RGB), JPEG and PNG
 * compression of individual frames 16-bit and 32-bit (float) images are
 * converted to 8-bit The plugin is based on the FileAvi class written by
 * William Gandler, part of Matthew J. McAuliffe's MIPAV program, available from
 * http://mipav.cit.nih.gov/. 2008-06-05: Support for jpeg and png-compressed
 * output and composite images by Michael Schmid. 2015-09-28: Writes AVI 2.0 if
 * the file size would be above approx. 0.9 GB
 *
 * The AVI format written looks like this: RIFF AVI RIFF HEADER, AVI CHUNK |
 * LIST hdrl MAIN AVI HEADER | | avih AVI HEADER | | LIST strl STREAM LIST(s)
 * (One per stream) | | | strh STREAM HEADER (Required after above; fourcc type
 * is 'vids' for video stream) | | | strf STREAM FORMAT (for video: BitMapInfo;
 * may also contain palette) | | | strn STREAM NAME | | | indx MAIN 'AVI 2.0'
 * INDEX of 'ix00' indices | LIST movi MOVIE DATA (maximum approx. 0.95 GB) | |
 * 00db or 00dc FRAME (b=uncompressed, c=compressed) | | 00db or 00dc FRAME | |
 * ... | | ix00 AVI 2.0-style index of frames within this 'movi' list RIFF AVIX
 * Only if required by size (this is AVI 2.0 extension) | LIST movi MOVIE DATA
 * (maximum approx. 0.95 GB) | | 00db or 00dc FRAME | | ... | | ix00 AVI
 * 2.0-style index of frames within this 'movi' list RIFF AVIX	further chunks,
 * each approx 0.95 GB (AVI 2.0) ...
 */
public class AVI_Writer implements PlugInFilter {
    ...
    private long[] sizePointers
            = //a stack of the pointers to the chunk sizes (pointers are
    ...
    /**
     * Reserve space to write the size of chunk and remember the position for a
     * later call to chunkEndWriteSize(). Several levels of chunkSizeHere() and
     * chunkEndWriteSize() may be nested.
     */
    ...
    /**
     * At the end of a chunk, calculate its size and write it to the position
     * remembered previously. Also pads to 2-byte boundaries.
     */
    ...
    /**
     * Write RGB data. Each 3-byte triplet in the bitmap array represents blue,
     * green, and red, respectively, for a pixel. The color bytes are in reverse
     * order (Windows convention). Lines are padded to a length that is a
     * multiple of 4 bytes.
     */
    ...
    /**
     * Write 8-byte int with Intel (little-endian) byte order (note:
     * RandomAccessFile.writeInt has other byte order than AVI)
     */
    ...
    /**
     * Write 4-byte int with Intel (little-endian) byte order (note:
     * RandomAccessFile.writeInt has other byte order than AVI)
     */
    ...
    /**
     * Write 2-byte short with Intel (little-endian) byte order (note:
     * RandomAccessFile.writeShort has other byte order than AVI)
     */
