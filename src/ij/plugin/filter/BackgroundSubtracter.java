 ...
 *
 * Imagine that the 2D grayscale image has a third (height) dimension by the
 * image value at every point in the image, creating a surface. A ball of given
 * radius is rolled over the bottom side of this surface; the hull of the volume
 * reachable by the ball is the background.
 *
 * With "Sliding Parabvoloid", the rolling ball is replaced by a sliding
 * paraboloid of rotation with the same curvature at its apex as a ball of a
 * given radius. A paraboloid has the advantage that suitable paraboloids can be
 * found for any image values, even if the pixel values are much larger than a
 * typical object size (in pixels). The paraboloid of rotation is approximated
 * as parabolae in 4 directions: x, y and the two 45-degree directions. Lines of
 * the image in these directions are processed by sliding a parabola against
 * them. Obtaining the hull needs the parabola for a given direction to be
 * applied multiple times (after doing the other directions); in this respect
 * the current code is a compromise between accuracy and speed.
 *
 * For noise rejection, with the sliding paraboloid algorithm, a 3x3 maximum of
 * the background is applied. With both, rolling ball and sliding paraboloid,
 * the image used for calculating the background is slightly smoothened (3x3
 * average). This can result in negative values after background subtraction.
 * This preprocessing can be disabled.
 *
 * In the sliding paraboloid algorithm, additional code has been added to avoid
 * subtracting corner objects as a background (note that a paraboloid or ball
 * would always touch the 4 corner pixels and thus make them background pixels).
 * This code assumes that corner particles reach less than 1/4 of the image size
 * into the image.
 *
 * Rolling ball code based on the NIH Image Pascal version by Michael Castle and
 * Janice Keller of the University of Michigan Mental Health Research Institute.
...
public class BackgroundSubtracter implements ExtendedPlugInFilter, DialogListener {
     ...
     *
     * @param ip The RGB image. On output, it will become the
     * background-subtracted image or the background (depending on
     * <code>createBackground</code>).
     * @param radius Radius of the rolling ball creating the background
     * (actually a paraboloid of rotation with the same curvature)
     * @param createBackground Whether to create a background, not to subtract
     * it.
     * @param lightBackground Whether the image has a light background.
     * @param doPresmooth Whether the image should be smoothened (3x3 mean)
     * before creating the background. With smoothing, the background will not
     * necessarily be below the image data.
     * @param correctCorners Whether the algorithm should try to detect corner
     * particles to avoid subtracting them as a background.
     */
     ...
     *
     * @param ip The image. On output, it will become the background-subtracted
     * image or the background (depending on <code>createBackground</code>).
     * @param radius Radius of the rolling ball creating the background
     * (actually a paraboloid of rotation with the same curvature)
     * @param createBackground Whether to create a background, not to subtract
     * it.
    ...
    /**
     * Create background for a float image by sliding a paraboloid over the
     * image.
     */
 ...
 /* Doing the diagonal directions at the end is faster (diagonal lines are denser,
    ...
    /**
     * Filter by subtracting a sliding parabola for all lines in one direction,
     * x, y or one of the two diagonal directions (diagonals are processed only
     * for half the image per call).
     */
    ...
    /**
     * Process one straight line in the image by sliding a parabola along the
     * line (from the bottom) and setting the values to make all points
     * reachable by the parabola
     *
     ...
     * @param coeff2 2nd order coefficient of the polynomial describing the
     * parabola, must be positive (although a parabola with negative curvature
     * is actually used)
     * @param cache Work array, length at least <code>length</code>. Will
     * usually remain in the CPU cache and may therefore speed up the code.
     * @param nextPoint Work array. Will hold the index of the next point with
     * sufficient local curvature to get touched by the parabola.
     * @param correctedEdges Should be a 2-element array used for output or
     * null.
     * @return The correctedEdges array (if non-null on input) with the two
     * estimated edge pixel values corrected for edge particles.
     */
...
class RollingBall {
    ...
    /**
     * Computes the location of each point on the rolling ball patch relative to
     * the center of the sphere containing it. The patch is located in the top
     * half of this sphere. The vertical axis of the sphere passes through the
     * center of the patch. The projection of the patch in the xy-plane below is
     * a square.
     */
