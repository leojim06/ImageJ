 ...
 * Plots (and lists) different curves as separate data sets. Works on all image
 * types (for RGB, uses gray value and threshold at 127.5)
 */
public class LineGraphAnalyzer implements PlugInFilter, Measurements {
     ...
     * Assumes the graph is thresholded or dark on white. Only analyzes pixels
     * inside the ROI
     */
    ...
    /**
     * Adds the average of yLo, yhi to the n-th FloatArray at a given (integer)
     * x value. A new FloatArray is created if n < 0. Missing data are filled
     * with NaN if necessary. Also writes the yLo and yHi values to the
     * curveValuesLo, curveValuesHi arrays at the corresponding index n
     */
     ...
     * Extrapolation is based on the last array value added and the slope of the
     * last values (never more than MAX_EXTRAPOLATE points back) A Extrapolator
     * instance must be provided (avoids unnnecessary garbage)
     */
    float extrapolated(FloatArray arr, int x, Extrapolator extrapolator) {
        extrapolator.clear();
        for (int i = 0, ix = x - 1; ix >= 0; ix--) {
            if (ix >= arr.size()) {
                continue; //we don't have a point at this x value
            }
            double y = arr.get(ix);
    ...
    /**
     * Extrapolator class. Extrapolation is based on the first point added and
     * the slope of a linear regression over all points added to the
     * Extrapolator.
     */
    class Extrapolator {
        ...
        /**
         * Gets the value of the extrapolation or NaN if undefined. If the x
         * value where we want the extrapolated value is close to the first
         * point added and no slope is avialable, assumes a slope of 0
         */
        public double extrapolate(double x) {
            ...
            if (Double.isNaN(slope) && Math.abs(x - firstX) <= 3) {
                slope = 0; //if we have no valid slope, assume slope=0 for nearby points
            }
