 ...
 * non-rectangular ROIs, which define the area where maxima are reported. Since
 * this plug-in creates a separate output image it processes only single images
 * or slices, no stacks.
 *
 * Notes: - When using one instance of MaximumFinder for more than one image in
 * parallel threads, all must images have the same width and height.
 *
 * version 09-Nov-2006 Michael Schmid version 21-Nov-2006 Wayne Rasband. Adds
 * "Display Point Selection" option and "Count" output type. version 28-May-2007
 * Michael Schmid. Preview added, bugfix: minima of calibrated images, uses
 * Arrays.sort version 07-Aug-2007 Fixed a bug that could delete particles when
 * doing watershed segmentation of an EDM. version 21-Apr-2007 Adapted for float
 * instead of 16-bit EDM; correct progress bar on multiple calls version
 * 05-May-2009 Works for images>32768 pixels in width or height version
 * 01-Nov-2009 Bugfix: extra lines in segmented output eliminated; watershed is
 * also faster now Maximum points encoded in long array for sorting instead of
 * separete objects that need gc New output type 'List' version 22-May-2011
 * Bugfix: Maximum search in EDM and float images with large dynamic range could
 * omit maxima version 13-Sep-2013 added the findMaxima() and findMinima()
 * functions for arrays (Norbert Vischer) version 20-Mar-2014 Watershed
 * segmentation of EDM with tolerance>=1.0 does not kill fine particles version
 * 11-Mar-2019 adds "strict" option, "noise tolerance" renamed to "prominence"
 */
public class MaximumFinder implements ExtendedPlugInFilter, DialogListener {

    //filter params
    /**
     * prominence: maximum height difference between points that are not counted
     * as separate maxima
     */
    private static double tolerance = 10;
    /**
     * strict=off allows one maximum even if it is not higher than the
     * prominence above all other pixels
     */
    ...
    /**
     * Set his to the number of images to process (for the watershed progress
     * bar only). Don't call or set nPasses to zero if no progress bar is
     * desired.
     */
    ...
    /**
     * Finds the image maxima and returns them as a Polygon, where poly.npoints
     * is the number of maxima. There is an example at<br>
     ...
     * @param excludeOnEdges Whether to exclude edge maxima. Also determines
     * whether strict mode is on, i.e., whether the global maximum is accepted
     * even if all other pixel are less than 'tolerance' below this level (In
     * 1.52m and before, 'strict' and 'excludeOnEdges' were the same).
     * @return A Polygon containing the coordinates of the maxima, where
     * poly.npoints is the number of maxima. Note that poly.xpoints.length may
     * be greater than the number of maxima.
     */
    ...
    /**
     * Finds the image maxima and returns them as a Polygon, where poly.npoints
     * is the number of maxima.
     *
     * @param ip The input image
     * @param tolerance Height tolerance: maxima are accepted only if protruding
     * more than this value from the ridge to a higher maximum
     * @param strict When off, the global maximum is accepted even if all other
     * pixel are less than 'tolerance' below this level. With
     * <code>excludeOnEdges=true</code>, 'strict' also means that the surounding
     * of a maximum within 'tolerance' must not include an edge pixel
     * (otherwise, it is enough that there is no edge pixel with the maximum
     * value).
     * @param excludeOnEdges Whether to exclude edge maxima. Also determines
     * whether strict mode is on, i.e., whether the global maximum is accepted
     * even if all other pixel are less than 'tolerance' below this level (In
     ...
     * @param edgeMode 0=include, 1=exclude, 3=circular edgeMode = 0 (include
     * edges) peak may be separated by one qualified valley and by a border.
     * edgeMode = 1 (exclude edges) peak must be separated by two qualified
     * valleys edgeMode = 2 (circular) array is regarded to be circular
     ...
     * IN_TOLERANCE or SEGMENTED. No output image is created for output types
     * POINT_SELECTION, LIST and COUNT.
     * @param excludeOnEdges Whether to exclude edge maxima. Also determines
     * whether strict mode is on, i.e., whether the global maximum is accepted
     * even if all other pixel are less than 'tolerance' below this level (In
     * 1.52m and before, 'strict' and 'excludeOnEdges' were the same).
     * @return A new byteProcessor with a normal (uninverted) LUT where the
     * marked points are set to 255 (Background 0). Pixels outside of the roi of
     * the input ip are not set. Returns null if outputType does not require an
     * output or if cancelled by escape
     */
     ...
     * segmentation lines may be improperly placed if local maxima are
     * suppressed by the tolerance.
     *
     * @param ip The input image
     * @param tolerance Height tolerance: maxima are accepted only if protruding
     * more than this value from the ridge to a higher maximum
     * @param threshold minimum height of a maximum (uncalibrated); for no
     * minimum height set it to ImageProcessor.NO_THRESHOLD
     * @param outputType What to mark in output image: SINGLE_POINTS,
     * IN_TOLERANCE or SEGMENTED. No output image is created for output types
     * POINT_SELECTION, LIST and COUNT.
     * @param excludeOnEdges Whether to exclude edge maxima. Also determines
    ...
    /**
     * Find all local maxima (irrespective whether they finally qualify as
     * maxima or not)
     *
     * @param ip The image to be analyzed
     * @param typeP A byte image, same size as ip, where the maximum points are
     * marked as MAXIMUM (do not use it as output: for rois, the points are
     ...
     * integer), the value is encoded in the upper 32 bits and the pixel offset
     * in the lower 32 bit Note: Do not use the positions of the points marked
     * as MAXIMUM in typeP, they are invalid for images with a roi.
     */
     ...
     * @param maxSortingError sorting may be inaccurate, sequence may be
     * reversed for maxima having values not deviating from each other by more
     * than this (this could be a result of precision loss when sorting ints
     * instead of floats, or because sorting does not take the height correction
     * in 'trueEdmHeight' into account
     * @param outputType
     */
     ...
     * threshold 0) and mark maximum areas as 255. For use as input for
     * watershed segmentation @param ip The original image that should be
     * segmented @param typeP Pixel t
     *
     * y
     * pes in ip @param isEDM Whether ip is an Euclidian distance map @param
     * globalMin The minimum pixel value of ip @param globalMax The maximum
     * pixel value of ip @param threshold Pixels of ip below this value
     *
     * (
     * c
     * a
     * librated) are considered background. Ignored if
     * ImageProcessor.NO_THRESHOLD @return The 8-bit output image.
     */
    ...
    /**
     * eliminate unmarked maxima for use by watershed. Starting from each
     * previous maximum, explore the surrounding down to successively lower
     * levels until a marked maximum is touched (or the plateau of a previously
     * eliminated maximum leads to a marked maximum). Then set all the points
     * above this value to this value
     *
     * @param outIp the image containing the pixel values
     * @param typeP the types of the pixels are marked here
     * @param maxPoints array containing the coordinates of all maxima that
     * might be relevant
     */
    ...
    /**
     * Delete extra structures form watershed of non-EDM images, e.g.,
     * foreground patches, single dots and lines ending somewhere within a
     * segmented particle Needed for post-processing watershed-segmented images
     * that can have local minima
     *
     ...
     * foreground. @param ip @param x coordinate of the point @param y
     * coordinate
     *
     * o
     * f
     * the point @return Number of 4-connected lines emanating from this point.
     * Zero if the point is embedded in either foreground or background
     */
    ...
    /**
     * after watershed, set all pixels in the background and segmentation lines
     * to 0
     */
    ...
    /**
     * delete a particle (set from value 255 to current fill value). Position
     * x,y must be within the particle
     */
     ...
     * set to 255 and the background set to 0. The image should not have any
     * local maxima other than the marked ones. Local minima will lead to
     * artifacts that can be removed later. On output, all particles will be set
     * to 255, segmentation lines remain at their old value.
     *
    ...
    /**
     * create an array of offsets within a pixel array for directions in
     * clockwise order: 0=(x,y-1), 1=(x+1,y-1), ... 7=(x-1,y) Also creates
     * further class variables: width, height, and the following three values
     * needed for storing coordinates in single ints for watershed:
     * intEncodeXMask, intEncodeYMask and intEncodeShift. E.g., for width
     * between 129 and 256, xMask=0xff and yMask = 0xffffff00 are bitwise masks
     * for x and y, respectively, and shift=8 is the bit shift to get y from the
     * y-masked value Returns as class variables: the arrays of the offsets to
     * the 8 neighboring pixels and the array maskAndShift for watershed
     */
