...
public class ScaleBar implements PlugIn {
     ...
     *
     * With or without the overlay, we can remove a scalebar drawn by this
    ...
    /**
     * If there is a user selected ROI, set the class variables {roiX} and
     * {roiY}, {roiWidth}, {roiHeight} to the corresponding features of the ROI,
    ...
    /**
     * There is no hard codded value for the width of the scalebar, when the
     * plugin is called for the first time in an ImageJ instance, a defautl
     * value for the width will be computed by this method.
     */
    void computeDefaultBarWidth(boolean currentROIExists) {
        ...
        if (currentROIExists && roiX >= 0 && roiWidth > 10) {
            ...
            if (config.hBarWidth > 0.67 * imageWidth) // If 80 pixels is too much, do 2/3 of the image.
            {
                config.hBarWidth = 0.67 * imageWidth;
            }
            if (config.hBarWidth > 5.0) // If the resulting size is larger than 5 units, round the value.
            {
        ...
        if (currentROIExists && roiY >= 0 && roiHeight > 10) {
            ...
            if (config.vBarHeight > 0.67 * imageHeight) // If 80 pixels is too much, do 2/3 of the image.
            {
                config.vBarHeight = 0.67 * imageHeight;
            }
            if (config.vBarHeight > 5.0) // If the resulting size is larger than 5 units, round the value.
            {
     ...
     *
     * Return the value of dialog.wasOKed() when the user clicks OK or Cancel.
     */
    ...
    /**
     * Store the active configuration into the static variable that is persisted
    ...
    /**
     * Sets the positions x y of hBackground and vBackground based on the
    ...
    /**
     * Sets the rectangles x y positions for scalebar elements (hBar, hText,
     * vBar, vText), based on the current configuration. Also sets the width and
     * height of the rectangles.
     *
     * The position of each rectangle is relative to hBackground and
     * vBackground, so setBackgroundBoxesPositions() must run before this method
     * computes positions. This method calls setBackgroundBoxesPositions().
     */
     ...
     *
     * This method chooses whether to use an overlay or the drawing tool to
     * create the scalebar.
     */
