...
public class ControlPanel implements PlugIn {
    ...
    /**
     * Creates a panel with the hierarchical tree structure of ImageJ's
     * commands.
     */
 ...
 /* *********************************************************************** */
     ...
     * {@link recursesubMenu(Menu, DefaultMutableTreeNode} method to gather the
     * root children.
     *
     */
    ...
    /**
     * Recursively builds up a tree structure from the Menu argument, by
     * populating the TreeNode argument with children TreeNode objects
     * constructed on the menu items. Descendants can be intermediate-level
     * nodes (submenus) or leaf nodes (i.e., no children). Leaf nodes will only
     * be added if there are any commands associated with them, i.e. their
     * labels correspond to keys in the hashtable returned by
     * <code>ij.Menus.getCommands()</code> except for the "Reload Plugins" menu
     * item, for which a local action command string is assigned to avoid
     * clashes with the action fired from ImageJ Plugins->Utilties->Reload
     * Plugins menu item.<br>
     * <strong>Note: </strong> this method bypasses the tree buildup based on
     * the {@link populateNode(Hashtable,DefaultMutableTreeNode)} method.
     *
     ...
     * collection. Actually, the method delegates to {@link populateNode{String[], String[], DefaultMutableTreeNode)}.
     *
     * @param collection Hashtable with `keys' (java.lang.String) representing a
     * tree path which is to be added to this node, but excludes it; the path
     * extends to the last child (leaf node). The `values' are
     * (java.lang.String) as labels and user objects for the last child of the
     * path. Actually, <code><strong>null<strong></code> elements for labels are
     * allowed, in which case the last child will be constructed on the last
     * token in the `key'.
    ...
    /**
     * Populates the <code>node</code> argument with items retrieved from the
     * two String[] arguments. Delegates indirectly to
     * {@link buildTreePath(String.String,String,DefaultMutableTreeNode)} to do
     * the job. If either arguments are empty (i.e., length=0) or have different
     * sizes, the method does nothing.
     *
     * @param items String array where each element is the source of a tree path
     * (see {
     * @see buildTreePath(String, String, DefaultMutableTreeNode)} and {
     * @see buildTreePath(String,String,String,DefaultMutableTreeNode)}.
     * @param labels String array where each element is the label of the root of
     * the tree path
     * @param node The TreeNode to be populated
     */
    ...
    /**
     * Short-hand for the four-argument <code>buildTreePath</code> method. Calls
     * <code>buildTreePath(source,label,<strong>null</strong>,topNode)</code>.
     *
     * @see buildTreePath(String,String,String,DefaultMutableTreeNode).
     *
     */
    ...
    /**
     * Builds up a tree path structure. Populates the <code>node</code> argument
     * with a tree path constructed as described below:
     *
     * @param source String to be parsed in a tree path; must be composed of
     * tokens delimited by "/"
     * @param label The label (String) of the leaf node for this path. If
     * <code><strong>null</strong></code> then the leafe nod will be constructed
     * from the last token.
     * @param command The command string of the action event fired upon clicking
     * the leaf node. If <code><strong>null</strong></code>, then the last token
 ...
 /* *********************************************************************** */
     ...
     *
     * @param s A string with the structure "[item1,item2,...,itemn]", as
     * returned by a call to the <code>toString()</code> method in the
 ...
 /* ************************************************************************** */
     ...
     * "Control_Panel". This is to facilitate the integration of these
     * properties with ImageJ's properties database. The keywords are
     * dot-separated lists of tokens; in each token, spaces are relaced by
     * underscores. Each token represents a node, and hence the keyword
     * represents a tree path. The values can be either:
     * <ol>
     * <li> a space-separated list of integers, indicating panel frame geometry
     * <strong>in the following fixed order:</strong>
     * x coordinate, y coordinate , width, height</li>
     * <li> or the word "expand" which indicates that the path represented by
     * the keyword is an expanded branch
     * </li>
     * </ol>
     *
     */
    ...
    void recordGeometry(TreePanel panel) {
        ...
        if (frame != null) {
            ...
            if (IJ.debugMode) {
                IJ.log("CP.recordGeometry: " + pTitle + " " + geometry);
            }
    ...
    void restoreGeometry(TreePanel panel) {
        ...
        if (IJ.debugMode) {
            IJ.log("CP.restoreGeometry: " + pTitle);
        }
...
// 	}
    ...
    // Thank you, Wayne!
    /**
     * Breaks the specified string into an array of ints. Returns null if there
     * is an error.
     */
...
class TreePanel implements
 ...
 /* ************************************************************************** */
     ...
     * panels (member of pcp). Then, if this is the only visible panel left, the
     * properties are saved; else, if this is the main panel, all other visible
     * panels are also closed and properties are saved
     */
    public void windowClosing(WindowEvent e) {
        if (IJ.debugMode) {
            IJ.log("CP.windowClosing: " + isMainPanel);
        }
        if (isMainPanel) {
            pcp.saveProperties();
        }
 ...
 /* ************************************************************************** */
    ...
    /**
     * Fires an ActionEvent upon double-click on the plugin item (leaf node) in
     * the JTree
     */
    void toAction() {
