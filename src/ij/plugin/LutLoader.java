 ...
 * greens and 256 blues), LUTs in text format, or generates the LUT specified by
 * the string argument passed to the run() method.
 */
public class LutLoader extends ImagePlus implements PlugIn {
    ...
    /**
     * Returns the LUT 'name' as an IndexColorModel, where 'name' can be any
     * entry in the Image/Lookup Tables menu.
     *
     * @see ij.IJ#getLuts See: Help>Examples>JavaScript/Show all LUTs
     */
    ...
    private FileInfo getBuiltInLut(String name) {
        ...
        if (name.equals("3-3-2 rgb")) {
            name = "3-3-2 RGB";
        }
        if (name.equals("red/green")) {
            name = "redgreen";
        }
        int nColors = 0;
        if (name.equals("fire")) {
    ...
    /**
     * Opens an NIH Image LUT, 768 byte binary LUT or text LUT from a file or
     * URL.
     */
    ...
    /**
     * Opens an ImageJ LUT using an InputStream and returns it as an
     * IndexColorModel. Since 1.43t.
     */
