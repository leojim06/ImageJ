...
/**
 * This plugin implements the Image/Stacks/Combine command. It combines two
 * stacks (w1xh1xd1 and w2xh2xd2) to create a new w1+w2 x max(h1,h2) x
 * max(d1,d2) stack. For example, a 256x256x40 and a 256x256x30 stack would be
 * combined into one 512x256x40 stack. If "Vertical" is checked, create a new
 * max(w1+w2) x (h1+h2) x max(d1,d2) stack. Unused areas in the combined stack
...
public class StackCombiner implements PlugIn {
    ...
    private ImageStack combine(ImageStack stack1, ImageStack stack2, boolean isHorizontal) {
        ...
        int w3 = isHorizontal ? (w1 + w2) : (Math.max(w1, w2));
        int h3 = isHorizontal ? (Math.max(h1, h2)) : (h1 + h2);
        ...
        for (int i = 1; i <= d3; i++) {
            ...
            if ((isHorizontal && (h1 != h2)) || (!isHorizontal && (w1 != w2))) {
            ...
            if (i <= d2) {
                int wip3 = isHorizontal ? w1 : 0;
                int hip3 = isHorizontal ? 0 : h1;
                ip3.insert(stack2.getProcessor(1), wip3, hip3);
