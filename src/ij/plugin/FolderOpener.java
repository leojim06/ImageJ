...
public class FolderOpener implements PlugIn, TextListener {
    ...
    /**
     * Opens the images in the specified directory as a stack. Opens the images
     * as a virtual stack if the 'options' string contains 'virtual' or 'use'.
     * Add ' filter=abc' to the options string to only open images with, for
     * example, 'abc' in their name. The image type, start, step, count and
     * scale can also be set, for example "bitdepth=32 start=10 step=2 count=10
     * scale=50". Add ' noMetaSort' to disable sorting of DICOM stacks by series
     * number (0020,0011). Displays the Import/Sequence dialog if the the 'path'
     * argument is null.
     */
     ...
     * 'virtual'. Add ' filter=abc' to the options string to only open images
     * with, for example, 'abc' in their name. The image type, start, step,
     * count and scale can also be set, for example "bitdepth=32 start=10 step=2
     * count=10 scale=50".
     */
    ...
    /**
     * Removes names that start with "." or end with ".db", ".txt", ".lut",
     * "roi", ".pty", ".hdr", ".py", etc.
     */
