...
/**
 * This plugin saves a 16 or 32 bit image in FITS format. It is a stripped-down
 * version of the SaveAs_FITS plugin from the collection of astronomical image
 * processing plugins by Jennifer West at
 * http://www.umanitoba.ca/faculties/science/astronomy/jwest/plugins.html.
 *
 * <br>Version 2010-11-23 : corrects 16-bit writing, adds BZERO & BSCALE updates
 * (K.A. Collins, Univ. Louisville).
 * <br>Version 2008-09-07 : preserves non-minimal FITS header if already present
 * (F.V. Hessman, Univ. Goettingen).
 * <br>Version 2008-12-15 : fixed END card recognition bug (F.V. Hessman, Univ.
 * Goettingen).
 * <br>Version 2019-11-03 : various updates (K.A. Collins, CfA-Harvard and
 * Smithsonian).
 */
    ...
    public void run(String path) {
        ...
        if (bitDepth == 8) {
            ...
            if (cal.getFunction() != Calibration.NONE && cal.getCoefficients() != null) {
                ...
                if (cal.getCoefficients()[1] != 0) {
                    bScale = cal.getCoefficients()[1];
                }
            }
...
//	}
     ...
     * ImagePlus image (or from the current slice label in the case of an
     * ImageStack) and returns it as an array of String objects representing
     * each card.
     *
     * Taken from the ImageJ astroj package
     * (www.astro.physik.uni-goettingen.de/~hessman/ImageJ/Astronomy)
     *
     * @param img	The ImagePlus image which has the FITS header in it's "Info"
     * property.
     */
