...
public class CommandFinder implements PlugIn, ActionListener, WindowListener, KeyListener, ItemListener, MouseListener {
    ...
    public CommandFinder() {
        ...
        if (interval == null) // Hopefully 300ms is a sensible default when the property
        // is not available.
        {
     ...
     *
     * @param initialSearch The search string that populates Command Finder's
     * search field. It is ignored if contains an invalid query (ie, if it is
     * either <tt>null</tt> or <tt>empty</tt>).
     */
