...
public class RoiManager extends PlugInFrame implements ActionListener, ItemListener, MouseListener, MouseWheelListener, ListSelectionListener, Iterable<Roi> {
    ...
    /**
     * Opens a single .roi file or a ZIP-compressed set of ROIs. Returns 'true'
    ...
    /**
     * If one ROI is selected, it is saved as a .roi file, if multiple (or no)
     * ROIs are selected, they are saved as a .zip ROI set. Returns 'true' if
     * the save operation was succesful.
     *
    ...
    /**
     * calculates the intersection of area, line and point selections. If there
     * is one PointRoi in the list of selected Rois, the points inside all
     * selected area rois are kept. If more than one PointRoi is selected, the
     * PointRois get converted to area rois with each pixel containing at least
     * one point selected.
     */
    void and() {
        ImagePlus imp = getImage();
    ...
    /**
     * Returns a reference to the ROI Manager if it open or opens it if it is
     * not already open.
     */
     ...
     * index is out of range. See also: RoiManager.getName() macro function.
     */
    ...
    /**
     * Returns the index of the first ROI with the specified name, or -1 if no
     * ROI has that name. See also: RoiManager.getIndex() macro function.
     */
