...
public class SyncWindows extends PlugInFrame implements
    ...
    /**
     * reference to current instance of ImageJ (to avoid repeated
     * IJ.getInstance() s)
     */
    ...
    //--------------------------------------------------
    /**
     * Create window sync frame. Frame is shown via call to show() or by
     * invoking run method.
     */
    public SyncWindows() {
    ...
    // --------------------------------------------------
    /**
     * Override parent windowClosing method to clean up synchronized resources
     * on exit.
     */
    ...
    /**
     * Implementation of ImageListener interface: update window list, if image
     * is opened or closed
     */
    ...
    // --------------------------------------------------
    /**
     * Build window list display and button controls. Create Hashtable that
     * connects list entries to window IDs.
     */
...
//		TODO: Give functionality to Synchronize Channels and Synchronize t-Frames checkboxes.
    ...
    // --------------------------------------------------
    /**
     * Compute bounding rectangle given current and old cursor locations. This
     * is used to determine what part of image to redraw.
     */
    ...
    // --------------------------------------------------
    /**
     * Adds "this" object as mouse listener and mouse motion listener to each of
     * the windows in input array.
     */
    ...
    /**
     * Get the title of image n from Windows-Vector vwins. If the image ends
     * with .tif, the extension is removed.
     */
    ...
    // --------------------------------------------------
    /**
     * Get Screen Coordinates for ImageCanvas ic matching the OffScreen
     * Coordinates of the current ImageCanvas. (srcRect and magnification stored
     * after each received event.) Input: The target ImageCanvas, the current
     * ImageCanvas, x-ScreenCoordinate for current Canvas, y-ScreenCoordinate
     * for current Canvas If the "ImageScaling" checkbox is selected, Scaling
     * and Offset of the images are taken into account.
     */
    ...
    // --------------------------------------------------
    /**
     * Makes a new mouse event from MouseEvent e with the Canvas c as source and
     * the coordinates of Point p as X and Y.
     */
...
class DisplayChangeEvent extends EventObject {
    ...
    /**
     * Type of change in display: Coordinate X, Y, Z, the Zoom, time, color
     * channel. So far there is no need for properties other than Z.
     */
...
class IJEventMulticaster extends AWTEventMulticaster implements DisplayChangeListener {
    ...
    /**
     * Adds DisplayChange-listener-a with DisplayChange-listener-b and returns
