...
//		maxSlider.addKeyListener(ij);
    ...
    /**
     * Called before each user interface action. Auto-thresholding is performed
     * if there is currently no threshold and 'enableAutoThreshold' is true.
     * Returns the ImageProcessor of the image that should be used, or null if
    ...
    /**
     * Whether the (auto)-thresholded pixels should be those with high values,
     * i.e., the background should be at low values. (E.g. dark background and
     * non-inverting LUT)
     */
    ...
    /**
     * Converts a number to a String, such that it should not take much space
     * (for the minLabel, maxLabel TextFields)
     */
    String d2s(double x) {
    ...
    /**
     * User has clicked 'Dark background'. Switch only if the current thresholds
     * are consistent with the previous 'Dark background' state.
     */
