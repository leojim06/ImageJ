...
public class Animator implements PlugIn {
    ...
    /**
     * Set 'arg' to "set" to display a dialog that allows the user to specify
     * the animation speed. Set it to "start" to start animating the current
     * stack. Set it to "stop" to stop animation. Set it to "next" or "previous"
    ...
    void changeSlice(int pn) {
        ...
        if (swin.getAnimate() && (channels * slices * frames == Math.max(channels, Math.max(slices, frames)))) {
            stopAnimation();
            return;
        } //if only one dimension, stop animating
        if (hyperstack) {
