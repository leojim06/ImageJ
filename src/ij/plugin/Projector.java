...
/**
 * This plugin creates a sequence of projections of a rotating volume (stack of
 * slices) onto a plane using nearest-point (surface), brightest-point, or
 * mean-value projection or a weighted combination of nearest- point projection
 * with either of the other two methods (partial opacity). The user may choose
 * to rotate the volume about any of the three orthogonal axes (x, y, or z),
 * make portions of the volume transparent (using thresholding), or add a
 * greater degree of visual realism by employing depth cues. Based on Pascal
 * code contributed by Michael Castle of the University of Michigan Mental
...
public class Projector implements PlugIn {
    ...
    private ImagePlus doProjections(ImagePlus imp) {
        ...
        if (debugMode) {
            if (projArray != null) {
                new ImagePlus("projArray", new ByteProcessor(projwidth, projheight, projArray, null)).show();
            }
            if (opaArray != null) {
                new ImagePlus("opaArray", new ByteProcessor(projwidth, projheight, opaArray, null)).show();
            }
            if (brightCueArray != null) {
                new ImagePlus("brightCueArray", new ByteProcessor(projwidth, projheight, brightCueArray, null)).show();
            }
            if (zBuffer != null) {
                new ImagePlus("zBuffer", new ShortProcessor(projwidth, projheight, zBuffer, null)).show();
            }
            if (cueZBuffer != null) {
                new ImagePlus("cueZBuffer", new ShortProcessor(projwidth, projheight, cueZBuffer, null)).show();
            }
            if (countBuffer != null) {
                new ImagePlus("countBuffer", new ShortProcessor(projwidth, projheight, countBuffer, null)).show();
            }
            if (sumBuffer != null) {
    ...
    /**
     * This method projects each pixel of a volume (stack of slices) onto a
     * plane as the volume rotates about the x-axis. Integer arithmetic,
     * precomputation of values, and iterative addition rather than
     * multiplication inside a loop are used extensively to make the code run
     * efficiently. Projection parameters stored in global variables determine
     * how the projection will be performed. This procedure returns various
     * buffers which are actually used by DoProjections() to find the final
     * projected image for the volume of slices at the current angle.
     */
    ...
    /**
     * Projects each pixel of a volume (stack of slices) onto a plane as the
     * volume rotates about the y-axis.
     */
    ...
    /**
     * Projects each pixel of a volume (stack of slices) onto a plane as the
     * volume rotates about the z-axis.
     */
