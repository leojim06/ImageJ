...
public class Macro_Runner implements PlugIn {
    ...
    /**
     * Opens and runs the specified macro file (.txt or .ijm) or script file
     * (.js, .bsh or .py) on the current thread. Displays a file open dialog if
     * <code>name</code> is an empty string. The macro or script is assumed to
     * be in the ImageJ plugins folder if  <code>name</code> is not a full path.
     */
    ...
    /**
     * Opens and runs the specified macro or script on the current thread. The
     * file is assumed to be in the ImageJ/macros folder unless 'name' is a full
     * path. The macro or script can use the getArgument() function to retrieve
     * the string argument.
     */
    ...
    /**
     * Runs the specified macro on the current thread. Macros can retrieve the
     * optional string argument by calling the getArgument() macro function.
     * Returns the string value returned by the macro, null if the macro does
     * not return a value, or "[aborted]" if the macro was aborted due to an
     * error.
     */
    ...
    /**
     * Runs the specified macro from a JAR file in the plugins folder, passing
     * it the specified argument. Returns the String value returned by the
     * macro, null if the macro does not return a value, or "[aborted]" if the
     * macro was aborted due to an error. The macro can reside anywhere in the
     * plugins folder, in or out of a JAR file, so name conflicts are possible.
     * To avoid name conflicts, it is a good idea to incorporate the plugin or
     * JAR file name in the macro name (e.g., "Image_5D_Macro1.ijm").
     */
    ...
    /**
     * Runs a JavaScript script on the current thread, passing a string
     * argument, which the script can retrieve using the getArgument() function.
     * Returns, as a string, the last expression evaluated by the script.
     */
     ...
     * as a string, the last expression evaluated by the script. Uses the plugin
     * at http://imagej.nih.gov/ij/plugins/bsh/ to run the script.
     */
     ...
     * which the script can retrieve using the getArgument() function. Returns,
     * as a string, the value of the variable 'result'. For example, a Python
     * script containing the line "result=123" will return the string "123".
     * Uses the plugin at http://imagej.nih.gov/ij/plugins/jython/ to run the
     * script.
     */
