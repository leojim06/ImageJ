 ...
 * surroundings. It is usefule for examining how a filter changes the data (also
 * during preview).
 *
 ...
 *
 * Radius determines the size of the window, 3x3 for radius=1, etc. The Pixel
 * Inspector window must be closed and opened to get the new size. Readout for
 * grayscale 8&16 bit images can be raw, calibrated or hexadecimal. Readout for
 * RGB images can ge R,G,B triples, gray value or hexadecimal. For copying the
 * data to clipboard, it can be selected whether the position (x,y) is not not
 * written, written in the first line or in the same way as the header lines of
 * the Pixel Inspector panel.
 *
 ...
 *
 * Michael Schmid Version 2007-Dec-06 - bugs fixed: did not always follow cursor
 * nudge could make the display hang pixel value calibration was sometimes
 * ignored Version 2007-Dec-14 - supports exponential format for large/small
 * data values
 */
...
class PixelInspector extends PlugInFrame
    ...
    /**
     * Converts a number to a string in decimal or exp format. The number of
     * digits is chosen to make the value fit into a cell the size of
     * "000000.00"
     */
