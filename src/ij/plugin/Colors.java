...
public class Colors implements PlugIn, ItemListener {
    ...
    /**
     * For named colors, returns the name, or 'defaultName' if not a named
     * color. If 'defaultName' is non-null and starts with an uppercase
     * character, the returned name is capitalized (first character uppercase).
     * Use colorToString or colorToString2 to get a String representation
     * (hexadecimal) also for unnamed colors.
     */
     ...
     * Returns 'defaultColor' if the color has no name. Use 'decode' to also
     * decode hex color names like "#ffff00"
     */
     ...
     * of a color with 6 or 8 hex digits to a Color. With 8 hex digits, the
     * first two digits are the alpha. With 6 hex digits, the color is opaque
     * (alpha = hex ff). A hex String may be preceded by '#' such as "#80ff00".
     * When the string does not include a valid color name or hex code, returns
     * Color.GRAY.
     */
     ...
     * first two digits are the alpha. With 6 hex digits, the color is opaque
     * (alpha = hex ff). A hex String may be preceded by "#" such as "#80ff00"
     * or "0x". When the string does not include a valid color name or hex code,
     ...
     * Returns null if the hex color does not have a name. Unused in ImageJ, for
     * compatibility only.
     */
     ...
     * "Red", "Green", "Yellow", etc. Returns null if the hex color does not
     * have a name. Unused in ImageJ, for compatibility only.
     */
    ...
    /**
     * Converts a Color into a lowercase string ("red", "green", "#aa55ff",
     * etc.). If <code>color</code> is <code>null</code>, returns the String
     * "none".
     */
    ...
    /**
     * Converts a Color into a string ("Red", "Green", #aa55ff, etc.). If
     * <code>color</code> is <code>null</code>, returns the String "None".
     */
    ...
    /**
     * Returns the 6-digit hex string such as "aa55ff" for opaque colors or or
     * 8-digit like "80aa55ff" for other colors (the first two hex digits are
     * alpha). 'color' must not be null.
     */
     ...
     * Values is outside the 0-255 range are replaced by the nearest valid
     * number (0 or 255)
     */
     ...
     * predefined color names "Red", "Green", ... "Pink". The Strings arguments
