...
public class ImagePlus implements ImageObserver, Measurements, Cloneable {
    ...
    /**
     * Locks the image so other threads can test to see if it is in use. One
     * thread can lock an image multiple times, then it has to unlock it as many
     * times until it is unlocked. This allows nested locking within a thread.
     * Returns true if the image was successfully locked. Beeps, displays a
     * message in the status bar, and returns false if the image is already
     ...
     * macros or other programs if an ImagePlus is shown on the screen, because
     * displaying the window is asynchronous (happens later) and will make the
     * image the active one. Without waiting, in the meanwhile another window
     * could be already the active one and would become deactivated. If the
     * ImagePlus may have been displayed previously, first call
     * setDeactivated(). <code>ImagePlus.show()</code> and
     * <code>new StackWindow(ImagePlus)</code> call this method if IJ.isMacro()
     * is true, i.e., when running a macro or executing an IJ.run(...) call.
     */
    ...
    /**
     * Called by ImageWindow.windowActivated(); to end waiting in
     * waitTillActivated.
     */
    ...
    /**
     * Extract pixels as an an ImageProcessor from a single band of a
     * BufferedImage.
     *
     * @param img
     * @param band
     ...
     * null. Mask pixels have a non-zero value.and the dimensions of the mask
     * are equal to the width and height of the ROI.
     *
    ...
    /**
     * Returns an 8-bit binary (foreground=255, background=0) ROI or overlay
     * mask that has the same dimensions as this image. Creates an ROI mask If
     * the image has both both an ROI and an overlay. Set the threshold of the
     * mask to 255.
     *
    ...
    /**
     * Returns an 8-bit binary threshold mask (foreground=255, background=0)
     * that has the same dimensions as this image. The threshold of the mask is
     * set to 255.
     *
    ...
    /**
     * Get calibrated statistics for this image or ROI, including histogram,
     * area, mean, min and max, standard deviation and mode. This code
     * demonstrates how to get the area, mean max and median of the current
     * image or selection:
     * <pre>
     * imp = IJ.getImage();
    ...
    /**
     * Returns an ImageStatistics object generated using the specified
     * measurement options and histogram bin count.
     */
    ...
    /**
     * Returns an ImageStatistics object generated using the specified
     * measurement options, histogram bin count and histogram range.
     */
    ...
    /**
     * If the image title is a file name, returns the name without the extension
     * and with spaces removed, otherwise returns the title shortened to the
     * first space.
     */
    ...
    /**
     * Adds a key-value pair to this image's string properties. The key-value
     * pair is removed if 'value' is null. The properties persist if the image
     * is saved in TIFF format. Add a "HideInfo" property (e.g.
     * set("HideInfo","true")) to prevent the properties from being displayed by
    ...
    /**
     * Returns as a string the image property associated with the specified key
    ...
    /**
     * Returns the numeric property associated with the specified key or NaN if
    ...
    /**
     * Returns a scaled copy of this image or ROI, where the 'options' string
     * can contain 'none', 'bilinear'. 'bicubic', 'average' and 'constrain'.
     */
    ...
    /**
     * Returns a cropped copy this image or stack, where 'options' can be
     * "stack", "slice", "whole-slice" or a range (e.g., "20-30").
     *
     * @see #duplicate
     * @see #crop
     * @see ij.plugin.Duplicator#crop
     ...
     * 'options' applies with stacks and can be "stack", "slice" or a range
     * (e.g., "20-30").
     *
     * @see #crop(ij.gui.Roi[])
     */
     ...
     * 'directory' is the directory path and 'format' is "tif", "png" or "jpg".
     * Set 'format' to "show" and the images will be displayed in a stack and
     * not saved.
     */
     ...
     * by ImageCanvas when the mouse moves.
     */
     ...
     * inserted into that selection, otherwise the selection is inserted into
    ...
    /**
     * Copies the contents of the internal clipboard to the specified location
     * using the specified transfer mode ("Copy", "Blend", "Average",
     * "Difference", "Transparent", "Transparent2", "AND", "OR", "XOR", "Add",
     * "Subtract", "Multiply", or "Divide"). The display is not updating.
     */
    ...
    /**
     * Copies the contents of the current selection, or the entire image if
     * there is no selection, to the system clipboard.
     */
     ...
     * it is identical to ip.setMinAndMax(min, max). Call updateAndDraw() to
     * update the display.
     */
    ...
    /**
     * Returns a "flattened" version of this image, or stack slice, in RGB
     * format.
     */
     ...
     * Copied from OverlayCommands and modified by Marcel Boeglin on 2014.01.08
     * to work with HyperStacks.
     */
