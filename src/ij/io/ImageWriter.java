...
public class ImageWriter {
    ...
    private int getCount(long imageSize) {
        ...
        count = (count / 4) * 4;
        if (IJ.debugMode) {
            IJ.log("ImageWriter: " + imageSize + " " + count + " " + imageSize / 50);
        }
        return count;
    }
    ...
    /**
     * Writes the image to the specified OutputStream. The OutputStream is not
     * closed. The fi.pixels field must contain the image data. If fi.nImages>1
     * then fi.pixels must be a 2D array, for example an array of images
     * returned by ImageStack.getImageArray()). The fi.offset field is ignored.
     */
