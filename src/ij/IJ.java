...
public class IJ {
     ...
     * does not return a value, or "[aborted]" if the macro was aborted due to
     ...
     * using the getArgument() macro function. Returns any string value returned
     ...
     * the called macro or script using the getArgument() function. Returns any
     * string value returned by the macro, or null. Scripts always return null.
    ...
    /**
     * Displays a message in the status bar and flashes either the status bar or
     * the active image.<br>
     ...
     * "StdDev", "Mode", "Min", "Max", "X", "Y", "XM", "YM", "Perim.", "BX",
     * "BY", "Width", "Height", "Major", "Minor", "Angle", "Circ.", "Feret",
     * "IntDen", "Median", "Skew", "Kurt", "%Area", "RawIntDen", "Ch", "Slice",
     * "Frame", "FeretX", "FeretY", "FeretAngle", "MinFeret", "AR", "Round",
     * "Solidity", "MinThr" or "MaxThr". Add " raw" to the argument to disable
     * calibration, for example IJ.getValue("Mean raw"). Add " limit" to enable
     ...
     * updated only if more than 90 ms have passed since the last call. Displays
     * subordinate progress bars as dots if 'currentIndex' is negative (example:
    ...
    /**
     * Displays a message in a dialog box titled "ImageJ". If a macro or
     ...
     * or JavaScript is running, it is aborted. Writes to the Java console if
    ...
    /**
     * Aborts any currently running JavaScript, or use IJ.error(string) to abort
     * a JavaScript with a message.
     */
    ...
    /**
     * Returns a string something like "64K of 256MB (25%)" that shows how much
     * of the available memory is in use. This is the string displayed when the
    ...
    /**
     * This is a version of setThreshold() that uses raw (uncalibrated) values
     * in the range 0-255 for 8-bit images and 0-65535 for 16-bit images and the
     * "Red" LUT display mode.
     */
     ...
     * "home" ("user.home"), "downloads", "startup", "imagej" (ImageJ
     * directory), "plugins", "macros", "luts", "temp", "current", "default",
     * "image" (directory active image was loaded from), "file" (directory most
     * recently used to open or save a file), "cwd" (current working directory)
     * or "preferences" (location of "IJ_Prefs.txt" file), otherwise displays a
     * dialog and returns the path to the directory selected by the user.
     * Returns null if the specified directory is not found or the user cancels
     * the dialog box. Also aborts the macro if the user cancels the dialog box.
     */
 ...
 /*
     ...
     * "white" , "black" (the default), "ramp", "composite-mode", "color-mode",
     ...
     * Tables menu.
     *
     * @see ij.plugin.LutLoader#getLut See also: Help>Examples>JavaScript/Show
     * all LUTs and Image/Color/Display LUTs
     */
