 ...
 * 0.5: Pixel (0,0) is enclosed by the rectangle between (0,0) and (1,1); thus
 * all 0.5 is added to all polygon coordinates when comparing to pixel
 * coordinates.
 *
 * After applying this offset, rounding is done such that points exactly on the
 * left boundary are considered outside, points exactly on the right boundary
 * inside. Points exactly on a horizontal boundary are considered ouside for the
 * boundary with lower y and inside for the boundary with the higher y.
...
public class PolygonFiller {
    ...
    /**
     * Constructs a PolygonFiller using the specified polygon with
    ...
    /**
     * Specifies the polygon to be filled in case of float coordinates. In this
     * case, multiple polygons separated by one set of NaN coordinates each.
     */
    ...
    /**
     * Generates the edge table for all non-horizontal lines: ey1, ey2: min &
     * max y value eslope: inverse slope dx/dy ex: x value at ey1, corrected for
     * half-pixel shift between outline&pixel coordinates sedge: list of sorted
     * edges is prepared (not sorted yet)
     */
    void buildEdgeTable() {
    ...
    /**
     * Shifts the x coordinates of all edges according to their slopes as
     * required for starting at the given y value and prepares the list of
     * active edges as it would have resulted from procesing the previous lines
     */
