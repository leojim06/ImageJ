 ...
 * types (byte, short, float and RGB) supported by ImageJ. An ImageProcessor
 * contains the pixel data of a 2D image and some basic methods to manipulate
 * it.
 *
 * @see ByteProcessor
 * @see ShortProcessor
...
public abstract class ImageProcessor implements Cloneable {
    ...
    /**
     * Inverts the values in this image's LUT (indexed color model). Does
     * nothing if this is a ColorProcessor.
     */
    ...
    /**
     * Returns 'true' if this is an image with a grayscale LUT or an RGB image
     * with identical red, green and blue channels.
     */
     ...
     * to screen values in the range 0-255. For byte images, this mapping is
     * done by updating the LUT. For short and float images, it's done by
     * generating 8-bit AWT images. For RGB images, it's done by changing the
     * pixel values. With signed 16-bit images, use
     ...
     * needed to correctly display the image. For ByteProcessors, resets the
     * LUT.
     */
    ...
    /**
     * Sets the lower and upper threshold levels. The 'lutUpdate' argument can
     * be RED_LUT, BLACK_AND_WHITE_LUT, OVER_UNDER_LUT or NO_LUT_UPDATE.
     ...
     * (over/under LUT) or 'no-lut' (no LUT changes), for example "Huang dark
     * b&w". The display range of 16-bit and 32-bit images is not reset if the
     * 'method' string contains 'no-reset'.
     *
    ...
    /**
     * Set the threshold using a 0-255 range. For 16-bit and 32-bit images, this
     * range is taken as relative with respect to the range between the current
     * display min and max, but lower=0 and upper=255 are set to the full-range
     * limits of 16-bit images and -/+1e30 for float images.
     */
    ...
    /**
     * Returns the lower threshold level. Returns NO_THRESHOLD if thresholding
     * is not enabled.
     */
    ...
    /**
     * Defines a rectangular region of interest and sets the mask to null if
     * this ROI is not the same size as the previous one.
     *
     ...
     * rectangular ROI and a mask. After processing, call
     * <code>reset(mask)</code> to restore non-masked pixels. Here is an
     * example:
     * <pre>
     * ip.setRoi(new OvalRoi(50, 50, 100, 50));
     * ip.fill();
     * ip.reset(ip.getMask());
     * </pre> The example assumes <code>snapshot()</code> has been called, which
     * is the case for code executed in the <code>run()</code> method of plugins
     * that implement the <code>PlugInFilter</code> interface.
     *
     ...
     * ROI and a mask. After processing, call <code>reset(mask)</code> to
     * restore non-masked pixels. Here is an example:
     * <pre>
    ...
    /**
     * Use this method to set the interpolation method (NONE, BILINEAR or
     * BICUBIC) used by scale(), resize() and rotate().
     */
     ...
     * (x1,y1) and ending at (x2,y2). Pixel values are sampled using
     * getInterpolatedValue(double,double) if interpolation is enabled or
     * getPixelValue(int,int) if it is not. For byte and short images, returns
     * calibrated values if a calibration table has been set using
     * setCalibrationTable(). The length of the returned array, minus one, is
     * approximately equal to the length of the line.
     *
    ...
    /**
     * Draws a string at the current drawing location using the current
     * fill/draw value. Draws multiple lines if the string contains newline
     * characters. The current x coordinate is the left/center/right end of the
     * string for left/center/right justification. The current y coordinate
     * determines the bottommost position of the string, including the descent
     * of the font (i.e., characters reaching below the baseline) For multi-line
     * strings, the y coordinate applies to the first line. The y of the drawing
     * position is incremented by the height of one text line, i.e. points at
     * the drawing position for the next text line
     */
     ...
     * adds the line height (FontMetrics.getHeight) to the current y coordinate
     * 'cy'
     */
    ...
    /**
     * Draws a string at the specified location using the current fill/draw
     * value. Draws multiple lines if the string contains newline characters.
     * The x coordinate is the left/center/right end of the string for
     * left/center/right justification. The y coordinate determines the
     * bottommost position of the string, including the descent of the font
     * (i.e., characters reaching below the baseline) For multi-line strings,
     * the y coordinate applies to the first line.
     */
    ...
    /**
     * Draws a string at the specified location with a filled background. A
     * JavaScript example is available at
     ...
     * <code>justification</code> is CENTER_JUSTIFY, RIGHT_JUSTIFY or
     * LEFT_JUSTIFY. The default is LEFT_JUSTIFY.
     */
     ...
     * test requires an 8 bit or RGB image. Antialiasing does not work with
    ...
    /**
     * Returns the width in pixels of the specified string, including any
     * background space (whitespace) between the x drawing coordinate and the
     * string, not necessarily including all whitespace at the right.
     */
    ...
    /**
     * Returns a rectangle enclosing the pixels affected by drawString assuming
     * it is drawn at (x=0, y=0). As drawString draws above the drawing
     * location, the y coordinate of the rectangle is negative.
     */
    ...
    /**
     * Rotates the entire image 90 degrees clockwise. Returns a new
    ...
    /**
     * Fills the image or ROI bounding rectangle with the current fill/draw
     * value. Use fill(Roi) or fill(ip.getMask()) to fill non-rectangular
     * selections.
     *
     * @see #setColor(Color)
     * @see #setValue(double)
    ...
    /**
     * Fills pixels that are within the ROI bounding rectangle and part of the
     * mask (i.e. pixels that have a value=BLACK in the mask array). Use
     * ip.getMask() to acquire the mask. Throws and IllegalArgumentException if
     * the mask is null or the size of the mask is not the same as the size of
     * the ROI.
     *
     * @see #setColor(Color)
     * @see #setValue(double)
    ...
    /**
     * Draws the specified ROI on this image using the line width and color
     * defined by ip.setLineWidth() and ip.setColor().
     *
    ...
    /**
     * Draws the specified ROI on this image using the stroke width, stroke
     * color and fill color defined by roi.setStrokeWidth, roi.setStrokeColor()
     * and roi.setFillColor(). Works with RGB images. Does not work with 16-bit
     * and float images.
     *
    ...
    /**
     * Draws the specified Overlay on this image. Works best with RGB images.
     * Does not work with 16-bit and float images. Requires Java 1.6.
     *
     * @see ImageProcessor#drawRoi
     */
     ...
     * convertToFloat() to calibrate pixel values. The length of the table must
     * be 256 for byte images and 65536 for short images. RGB and float
     * processors do not do calibration.
     *
    ...
    /**
     * Returns the number of float image histogram bins. The bin count is fixed
     * at 256 for the other three data types.
     */
     ...
     * is out of range. Use <i>getValue(x,y)</i> to get calibrated values from
     * 8-bit and 16-bit images, to get intensity values from RGB images and to
     * get float values from 32-bit images.
     *
     ...
     * getPixelValue() but does no bounds checking and does not return
     * calibrated values.
     */
    ...
    /**
     * Sets the value of the pixel at (x,y) to 'value'. Does no bounds checking
     * or clamping, making it faster than putPixel(). Due to the lack of bounds
     * checking, (x,y) coordinates outside the image may cause an exception. Due
     * to the lack of clamping, values outside the 0-255 range (for byte) or
     * 0-65535 range (for short) are not handled correctly.
     */
    ...
    /**
     * Returns a copy of the pixel data as a 2D int array with dimensions
     * [x=0..width-1][y=0..height-1]. With RGB images, the returned values are
     * in packed ARGB format. With float images, the returned values must be
     * converted to float using Float.intBitsToFloat().
     */
    ...
    /**
     * Returns a copy of the pixel data as a 2D float array with dimensions
    ...
    /**
     * Returns the samples for the pixel at (x,y) in an int array. RGB pixels
     * have three samples, all others have one. Returns zeros if the the
     * coordinates are not in bounds. iArray is an optional preallocated array.
     */
     ...
     */
    /**
     * Uses the current interpolation method (bilinear or bicubic) to find the
     ...
     * coordinates (x,y). For RGB images, the argb values are packed in an int.
     * For float images, the value must be converted using
     * Float.intBitsToFloat(). Returns zero if the (x, y) is not inside the
     * image.
     */
     ...
     * clamped. For RGB images, the argb values are packed in 'value'. For float
    ...
    /**
     * Returns the value of the pixel at <i>(x,y)</i>, a calibrated value from
     * 8-bit and 16-bit images, an intensity value from RGB images and a double
     * value from 32-bit images. This is an alias for getPixelValue(x,y).
     *
    ...
    /**
     * Returns the value of the pixel at (x,y). For byte and short images,
     * returns a calibrated value if a calibration table has been set using
    ...
    /**
     * Sets a new pixel array for the image. The length of the array must be
     * equal to width*height. Use setSnapshotPixels(null) to clear the snapshot
     * buffer.
     */
    ...
    /**
     * Adds pseudorandom, Gaussian ("normally") distributed values, with mean
     * 0.0 and the specified standard deviation, to this image or ROI.
     */
    public abstract void noise(double standardDeviation);

    /**
     * Returns a new processor containing an image that corresponds to the
     * current ROI.
     */
    ...
    /**
     * Scales the image by the specified factors. Does not change the image
     * size.
     *
     * @see ImageProcessor#setInterpolate
    ...
    /**
     * Returns a new ImageProcessor containing a scaled copy of this image or
     * ROI.
     *
    ...
    /**
     * Returns a new ImageProcessor containing a scaled copy of this image or
     * ROI, with the aspect ratio maintained.
     */
     ...
     * artifacts; the kernel shape for averaging is determined by the
     * interpolationMethod. False if subsampling without any averaging should be
     * used on downsizing. Has no effect on upsizing.
    ...
    /**
     * Use linear interpolation to resize images that have a width or height of
     * one.
     */
     ...
     * possible value that a pixel can have, from 0 to 255 (8-bit image) or
     * 0-65535 (16-bit image). Thus, the array size is 256 or 65536, and the bin
     * width in uncalibrated units is 1.
     * <p>
     * For RGB images, the brightness is evaluated using the color weights
     * (which would result in a float value) and rounded to an int. This gives
     * 256 bins. FloatProcessor.getHistogram is not implemented (returns null).
     */
    ...
    /**
     * Dilates the image or ROI using a 3x3 minimum filter. Requires 8-bit or
     * RGB image.
     */
    public abstract void dilate();

    /**
     * For 16 and 32 bit processors, set 'lutAnimation' true to have
     * createImage() use the cached 8-bit version of the image.
     */
    ...
    /**
     * Returns a 32-bit float version of this image as a FloatProcessor. For
     * byte and short images, converts using a calibration function if a
    ...
    /**
     * Returns an 8-bit version of this image as a ByteProcessor. 16-bit and
     * 32-bit pixel data are scaled from min-max to 0-255.
     */
    ...
    /**
     * Returns an 8-bit version of this image as a ByteProcessor. 16-bit and
     * 32-bit pixel data are scaled from min-max to 0-255 if 'scale' is true.
     */
    ...
    /**
     * Returns a 16-bit version of this image as a ShortProcessor. 32-bit pixel
     * data are scaled from min-max to 0-255.
     */
    ...
    /**
     * Returns a 16-bit version of this image as a ShortProcessor. 32-bit pixel
     * data are scaled from min-max to 0-255 if 'scale' is true.
     */
     ...
     * determined threshold. For RGB images, converts each channel to binary.
     * For float images, does nothing.
     */
     ...
     * into objects and background. It does this by taking a test threshold and
     * computing the average of the pixels at or below the threshold and pixels
     * above. It then computes the average of those two, increments the
     * threshold, and repeats the process. Incrementing stops when the threshold
     * is larger than the composite average. That is, threshold = (average
     * background + average objects)/2. This description was posted to the
     * ImageJ mailing list by Jordan Bevic.
     */
    ...
    /**
     * This is a version of getAutoThreshold() that uses a histogram passed as
     * an argument.
     */
    ...
    /**
     * Updates the clipping rectangle used by lineTo(), drawLine(), drawDot()
     * and drawPixel(). The clipping rectangle is reset by passing a null
     * argument or by calling resetRoi().
     */
    ...
    /**
     * The getPixelsCopy() method returns a reference to the snapshot buffer if
     * it is not null and 'snapshotCopyMode' is true.
     *
    ...
    /**
     * This method is used by CompositeImage.updateImage() to create RGB images
     * (for display) of a multi-channel composite images.
     */
    ...
    /**
     * Returns 'true' if this is a binary image (8-bit-image with only 0 and
     * 255).
     */
     ...
     * including histogram, area, mean, min and max, standard deviation, and
     * mode. Use the setRoi(Roi) method to limit statistics to a non-rectangular
     * area.
     *
    ...
    /**
     * Calculates and returns complete uncalibrated (raw) statistics for this
     * image or ROI but it is up to 70 times slower than getStats(). Use the
    ...
    /**
     * Returns a shallow copy of this ImageProcessor, where this image and the
     * copy share pixel data. Use the duplicate() method to create a copy that
     * does not share the pixel data.
     *
    ...
    /**
     * Returns a binary mask, or null if a threshold is not set or this is an
     * RGB image.
     *
