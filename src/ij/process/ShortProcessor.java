...
/**
 * ShortProcessors contain a 16-bit unsigned image and methods that operate on
 * that image.
 */
public class ShortProcessor extends ImageProcessor {
    ...
    /**
     * @deprecated 16 bit images are normally unsigned but signed images can be
     * simulated by subtracting 32768 and using a calibration function to
    ...
    /**
     * Obsolete. 16 bit images are normally unsigned but signed images can be
     * used by subtracting 32768 and using a calibration function to restore the
     * original values.
     */
     ...
     * mapped to 0-255 screen values. With signed 16-bit images, use
    ...
    /**
     * Recalculates the min and max values used to scale pixel values to 0-255
     * for display. This ensures that this ShortProcessor is set up to correctly
     * display the image.
     */
    ...
    /**
     * Uses the current interpolation method (BILINEAR or BICUBIC) to calculate
    ...
    /**
     * Stores the specified real value at (x,y). Does nothing if (x,y) is
     * outside the image boundary. Values outside the range 0-65535
     * (-32768-32767 for signed images) are clipped. Support for signed values
     * requires a calibration table, which is set up automatically with
     * PlugInFilters.
     */
    ...
    /**
     * Returns a reference to the short array containing this image's pixel
     * data. To avoid sign extension, the pixel values must be accessed using a
     * mask (e.g. int i = pixels[j]&0xffff).
     */
     ...
     * the mask is not the same as the ROI.
     */
    ...
    /**
     * Returns 65,536 bin histogram of the current ROI, which can be
     * non-rectangular.
     */
    ...
    /**
     * Creates a histogram of length maxof(max+1,256). For small images or
     * selections, computations using these histograms are faster compared to
     * 65536 element histograms.
     */
    int[] getHistogram2() {
        if (mask != null) {
    ...
    /**
     * Sets the pixels from a FloatProcessor, no scaling. Also the min&max
     * values are taken from the FloatProcessor.
     *
     * @param channelNumber Ignored (needed for compatibility with
     * ColorProcessor.toFloat)
