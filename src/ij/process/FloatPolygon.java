...
/**
 * Used by the Roi classes to return float coordinate arrays and to determine if
 * a point is inside or outside of spline fitted selections.
 */
public class FloatPolygon {
     ...
     *
     * In the absence of numerical errors, x coordinates exactly at a boundary
     * are taken as if in the area immediately at the left of the boundary. For
     * horizontal boundaries, y coordinates exactly at the border are taken as
     * if in the area immediately above it (i.e., in decreasing y direction).
     * The ImageJ convention of an offset of 0.5 pixels between pixel centers
     * and outline coordinates is not taken into consideration; this method
     * returns the purely geometrical relationship.
     */
     ...
     * this FloatPolygon and returns it as a new FloatPolygon. The sequence of
     * the points on the convex hull is counterclockwise.
     */
    public FloatPolygon getConvexHull() {
        ...
        do {
            ...
            do {
                ...
                if (p2 == p1) {
                    break;           // all points have the same coordinates as p1
                }
            ...
            if (p2 != p1) {
                do {
                    ...
                    if (determinate > 0 || collinearAndFurther) {
                        x2 = x3;
                        y2 = y3;
                        p2 = p3;       // p2 is not on the convex hull, p3 becomes the new candidate
                    }
                    p3++;
                    if (p3 == npoints) {
                        p3 = 0;
            ...
            if (p2 == p1) {
                break;               // happens only if there was only one unique point
            }
            p1 = p2;
            if (n2 > 1 && xpoints[p1] == xx[0] && ypoints[p1] == yy[0]) {
                break; //all done but pstart was missed because of duplicate points
            }
        } while (p1 != pstart);
