...
public class ByteProcessor extends ImageProcessor {
    ...
    /**
     * Creates a ByteProcessor from an ImageProcessor. 16-bit and 32-bit pixel
     ...
     * outside the image boundary. Values outside the range 0-255 are clamped.
     */
     ...
     * image boundary. Values outside the range 0-255 are clamped.
     */
     ...
     * contain the values of the pixels in the neighborhood, are arranged as
     * follows:
     * <pre>
     * p1 p2 p3
     * p4 p5 p6
    ...
    void filterEdge(int type, byte[] pixels2, int n, int x, int y, int xinc, int yinc) {
        ...
        for (int i = 0; i < n; i++) {
            if ((!Prefs.padEdges && type == ERODE) || type == DILATE) {
                ...
                p1 = getEdgePixel1(pixels2, fg, x - 1, y - 1);
                p2 = getEdgePixel1(pixels2, fg, x, y - 1);
                p3 = getEdgePixel1(pixels2, fg, x + 1, y - 1);
                p4 = getEdgePixel1(pixels2, fg, x - 1, y);
                p5 = getEdgePixel1(pixels2, fg, x, y);
                p6 = getEdgePixel1(pixels2, fg, x + 1, y);
                p7 = getEdgePixel1(pixels2, fg, x - 1, y + 1);
                p8 = getEdgePixel1(pixels2, fg, x, y + 1);
                p9 = getEdgePixel1(pixels2, fg, x + 1, y + 1);
            } else {
    ...
    /**
     * Converts objects with pixel values of 'forground' (255 or 0) in a binary
     * imager to single pixel skeletons.
     */
