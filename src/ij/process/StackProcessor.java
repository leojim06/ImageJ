...
public class StackProcessor {
    ...
    /**
     * Constructs a StackProcessor from a stack. 'ip' is the processor that will
     * be used to process the slices. 'ip' can be null when using crop().
     */
    ...
    void process(int command) {
        ...
        switch (command) {
            ...
            case SCALE:
                s = "Scale: ";
                break;
            case SCALE_WITH_FILL:
                s = "Scale: ";
                ip2.setBackgroundValue(fillValue);
                break;
            case INVERT:
                s = "Invert: ";
                break;
        ...
        for (int i = 1; i <= nSlices; i++) {
            ...
            switch (command) {
                ...
                case SCALE:
                case SCALE_WITH_FILL:
                    ip2.scale(xScale, yScale);
                    break;
                case INVERT:
                    ip2.invert();
                    break;
                case APPLY_TABLE:
                    ip2.applyTable(table);
                    break;
            }
    ...
    /**
     * Creates a new stack with dimensions 'newWidth' x 'newHeight'. To reduce
     * memory requirements, the orginal stack is deleted as the new stack is
     * created.
     */
        ...
        for (int z = zmin; z < zmax; z++) {
            ...
            for (int y = 0; y < sizey; y++) {
                for (int x = 0; x < sizex; x++) {
                    ...
                    switch (filter) {
                        case FILTER_MEAN:
                            out.setVoxel(x, y, z, tab.getMean());
                            break;
                        case FILTER_MEDIAN:
                            out.setVoxel(x, y, z, tab.medianSort());
                            break;
                        case FILTER_MIN:
                            out.setVoxel(x, y, z, tab.getMinimum());
                            break;
                        case FILTER_MAX:
                            out.setVoxel(x, y, z, tab.getMaximum());
                            break;
                        case FILTER_VAR:
                            out.setVoxel(x, y, z, tab.getVariance());
                            break;
