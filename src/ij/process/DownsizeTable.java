...
/**
 * A table for easier downsizing by convolution with a kernel. Supports the
 * interpolation methods of ImageProcessor: none, bilinear, bicubic Convention
 * used: The left edges of the first pixel are the same for source and
 * destination. E.g. when downsizing by a factor of 2, pixel 0 of the
 * destination takes the space of pixels 0 and 1 of the source.
 *
 * Example for use: Downsizing row 0 of 'pixels' from 'roi.width' to
 * 'destinationWidth'. The input range is given by the roi rectangle. Output is
 * written to row 0 of 'pixels2' (width: 'destinationWidth')  <code>
...
public class DownsizeTable {
     ...
     * points where the data should be added. Arranged in blocks of 'kernelSize'
     * points. E.g. for kernelSize=2, array elements 0,1 are for point srcStart,
     * 2,3 for point srcStart+1, etc.
     */
    ...
    /**
     * Create a table for 1-dimensional downscaling interpolation. Interpolation
     * is done by
     *
     * @param srcSize	Size of source data, i.e., width or height of input image
     * @param srcOrigin	Index of first pixel of source data that corresponds to
     * an ouput pixel, 0 or origin of source rectangle if only a roi is scaled
     * @param srcLength	Number of pixels of source data that should correspond
     * to output, i.e., width or height of source roi
