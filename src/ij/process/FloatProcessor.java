...
public class FloatProcessor extends ImageProcessor {
    ...
    /**
     * Creates a FloatProcessor from an int array using the default grayscale
     * LUT.
     */
     ...
     * Returns without doing anything if fixedScale has been set true as a
     * result of calling setMinAndMax(). In this case, getMin() and getMax()
     * return the fixed min and max defined by setMinAndMax(), rather than the
     ...
     * mapped to 0-255 screen values. Use resetMinAndMax() to enable
     * auto-scaling;
     *
     ...
     * for display. This ensures that this FloatProcessor is set up to correctly
     * display the image.
     */
     ...
     * iArray is an optiona preallocated array.
     */
    ...
    /**
     * Returns a reference to the float array containing this image's pixel
     * data.
     */
    ...
    /**
     * Each pixel in the image is inverted using p=max-(p-min), where 'min' and
     * 'max' are the display range limits set using setMinAndMax().
     */
    public void invert() {
        process(INVERT, 0.0);
    }
     ...
     * @param fp	Ignored (needed for compatibility with the other ImageProcessor
     * types).
    ...
    /**
     * Sets the pixels, and min&max values from a FloatProcessor. Also the
