...
public class FHT extends FloatProcessor {
    ...
    /**
     * Constructs a FHT object from an ImageProcessor. Byte, short and RGB
     * images are converted to float. Float images are duplicated.
     */
    ...
    /**
     * Returns true of this FHT contains a square image with a width that is a
     * power of two.
     */
     ...
     * domain. The image contained in this FHT must be square and its width must
     * be a power of 2.
     */
     ...
     * Transform. With no Window function, if the array size is a power of 2,
     * the input function should be either periodic or the data at the beginning
     * and end of the array should approach the same value (the periodic
     * continuation should be smooth). With no Window function, if the array
     * size is not a power of 2, the data should decay towards 0 at the
     * beginning and end of the array. For data that do not fulfill these
     * conditions, a window function can be used to avoid artifacts from the
     * edges. See http://en.wikipedia.org/wiki/Window_function.
     *
     * Supported window functions: Hamming, Hann ("raised cosine"), flat-top.
     * Flat-top refers to the HFT70 function in the report cited below, it is
     * named for its response in the frequency domain: a single-frequency
     ...
     * frequency resolution; this is especially pronounced for the flat-top
     * window.
     *
     ...
     * amplitude/sqrt(2)), and the first Fourier amplitude corresponds to DC
     * component (average value of the data). If the sine frequency falls
     * between two discrete frequencies of the Fourier transform, peak heights
     * can deviate from the true RMS amplitude by up to approx. 36, 18, 15, and
     * 0.1% for no window function, Hamming, Hann and flat-top window functions,
     * respectively. When calculating the power spectrum from the square of the
     * output, note that the result is quantitative only if the input array size
     * is a power of 2; then the spectral density of the power spectrum must be
     * divided by 1.3628 for the Hamming, 1.5 for the Hann, and 3.4129 for the
     * flat-top window.
     *
     * For more details about window functions, see: G. Heinzel, A. Rdiger, and
     * R. Schilling Spectrum and spectral density estimation by the discrete
     * Fourier transform (DFT), including a comprehensive list of window
     * functions and some new flat-top windows. Technical Report, MPI f.
     * Gravitationsphysik, Hannover, 2002; http://edoc.mpg.de/395068
     *
     * @param data Input array; its size need not be a power of 2. The input is
     * not modified..
     * @param windowType may be NO_WINDOW, then the input array is used as it
     * is. Otherwise, it is multiplied by a window function, which can be
     * HAMMING, HANN or FLATTOP.
     * @return Array with the result, i.e., the RMS amplitudes for each
     * frequency. The output array size is half the size of the 2^n-sized array
     * used for the FHT; array element [0]corresponds to frequency zero (the "DC
     * component"). The first nonexisting array element, result[result.length]
     * would correspond to a frequency of 1 cycle per 2 input points, i.e., the
     * Nyquist frequency. In other words, if the spacing of the input data
     * points is dx, results[i] corresponds to a frequency of
     * i/(2*results.length*dx).
     */
    ...
    /**
     * Performs an optimized 1D Fast Hartley Transform (FHT) of an array. Array
     * size must be a power of 2. Note that all amplitudes in the output 'x' are
     * multiplied by the array length. Therefore, to get the power spectrum, for
     * 1 <=i < N/2, use
     ...
     *  and for i>0, use (x[i]+x[maxN-i])/(2*maxN). The imaginary part of the
     * complex FFT, with i>0, is given by (x[i]-x[maxN-i])/(2*maxN) The
     * coefficients of cosine and sine are like the real and imaginary values
     * above, but you have to divide by maxN instead of 2*maxN.
     */
     ...
     * @param base First index from where data of the input array should be
     * read.
     * @param inverse True for inverse transform.
     * @param maxN Length of data that should be transformed; this must be
     * always the same for a given FHT object. Note that all amplitudes in the
     * output 'x' are multiplied by maxN.
     */
     ...
     * slice stack. Author: Joachim Wesner
     */
    ...
    /**
     * FFT real value of one row from 2D Hartley Transform. Author: Joachim
     * Wesner
     */
    ...
    /**
     * FFT imag value of one row from 2D Hartley Transform. Author: Joachim
     * Wesner
     */
    ...
    /**
     * Swap quadrants 1 and 3 and 2 and 4 of the specified ImageProcessor so the
    ...
    /**
     * Returns the image resulting from the point by point Hartley
     * multiplication of this image and the specified image. Both images are
     * assumed to be in the frequency domain. Multiplication in the frequency
     * domain is equivalent to convolution in the space domain.
     */
     ...
     * multiplication of this image and the specified image. Both images are
     * assumed to be in the frequency domain. Conjugate multiplication in the
     * frequency domain is equivalent to correlation in the space domain.
     */
    ...
    /**
     * Returns the image resulting from the point by point Hartley division of
     * this image by the specified image. Both images are assumed to be in the
     * frequency domain. Division in the frequency domain is equivalent to
